...
核心思路：利用高敏感配置文件对文件内容格式的容错性以及写入后立即生效特性，构造恶意内容写入文件，从而实现提权。
...
如果是 sudo 文件，能构造出换行并控制写入其中一行内容，那么直接覆盖整个文件。如果不是二进制文件或没有使用 `#!` 指定解释器，默认作为 shell 脚本解析，bash 脚本默认遇到错误行仍然会继续往下走。
