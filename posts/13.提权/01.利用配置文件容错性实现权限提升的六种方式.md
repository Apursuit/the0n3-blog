...
description: >-
  Linux 提权技巧：利用配置文件逐行解析、忽略错误行的特性，在写入内容不完全可控时构造换行注入一行有效配置即可提权，涵盖覆写 sudo 文件、authorized_keys、cron、sudoers、passwd、shadow、group、ld.so.preload 八种方式。
---
...
测试存在无效行时，篡改 root 行哈希为已知明文为123456 的 MD5-crypt 哈希，测试可以正常登录到 root
