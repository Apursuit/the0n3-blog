---
title: Linux近期提权漏洞汇总：copyfail、dirtyfray、Fragnesia、Pack2theroot
date: 2026-05-12 19:32:58
permalink: /posts/8da3f0/
tags: []
categories: 
- CVE
- LPE本地提权
...
[Copy Fail - 摘自维基百科，自由的百科全书](https://en.wikipedia.org/wiki/Copy_Fail)
...
copy-fail-c-<arch>-<libc>：二进制变异变体。变异setuid 二进制文件的页面缓存，然后执行它。

copy-fail-c-passwd-<arch>-<libc>/etc/passwd UID 翻转变体。修改 /etc/passwd 页面缓存中的四个字节，然后执行。在二进制修改路径被阻塞的情况下有效，但缓存范围较窄；详情请参阅 README 文件。
...
![](/images/amazingCVE2026/dirtyfrag.png)
...
![](/images/amazingCVE2026/Fragnesia.png)

Fragnesia 是一个通用的 Linux 本地权限提升漏洞，由 [William Bowling](https://x.com/wcbowling?lang=en) 与 [V12 团队](https://x.com/v12sec) 在 [V12](https://v12.sh) 中发现。Fragnesia 属于 [Dirty Frag](https://github.com/V4bel/dirtyfrag) 漏洞类别的一员。它是 ESP/XFRM 中与 dirtyfrag **不同的一个独立漏洞**，并且已经获得了[自己的补丁](https://lists.openwall.net/netdev/2026/05/13/79)。不过，它位于相同的攻击面上，且缓解措施与 dirtyfrag 相同。
...
项目提供了一个 C 语言版本的 PoC，针对`/usr/bin/su`文件，编译后直接运行即可触发漏洞并获得 root 权限。
...
![](/images/amazingCVE2026/suid1.png)

![](/images/amazingCVE2026/suid2.png)
