...
本文基于 BusyBox 1.38.0 源码 、 Alpine Linux 3.20.0 环境进行分析
...
本实验基于 Alpine Linux 轻量级容器环境快速复现
...
### 1.2 现象：SUID Bash 下的 "提权失败"

实验环境为 **Alpine Linux 3.20.0**。Alpine 以 BusyBox 作为核心系统工具集，`/bin/sh`、`/bin/ls`、`/bin/id` 等绝大多数基础命令均为指向 `/bin/busybox` 的符号链接。每个这样的命令称为一个 **applet**——BusyBox 通过 `argv[0]` 识别要执行哪个 applet，并在其 `main()` 之前运行统一的 `check_suid()` 进行特权管理

实验环境：Alpine Linux 3.20.0、Linux kernel 6.6.0、BusyBox 1.38.0-r0、musl libc 1.2.5、/bin/busybox 无 SUID 位、bbsuid 启用、/var/tmp 非 nosuid 挂载
...
进入 `id_main()` 前，`check_suid()` 已执行 `setuid(1000)` 降权。整个调用链如下——**主动特权的是 `id` 这个 applet 自己，不是 `bash -p` 那个 shell**：
