...
时序关键：check_suid() 在 applet_main() 之前执行。applet 的 main() 函数拿到的是已经降权后的身份
...
static void check_suid(int applet_no)
{
    ...
    // ── 分支 2：BB_SUID_DROP → 主动丢弃特权 ──
        ...
        // 此时: ruid=demo, euid=demo, suid=demo  ← 三个全变了
    }
...
### 3.3 sanitize_env_if_suid() — 环境变量清理
...
该函数并非全局自动调用，只有 3 个 applet 显式调用它
