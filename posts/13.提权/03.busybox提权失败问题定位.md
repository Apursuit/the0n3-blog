...
答案在 `check_suid()` 的三种模式里。

### 1.5 check_suid() 的三种模式
...
| 场景 | ruid | euid | suid | DROP 类 | REQUIRE 类 | MAYBE 类 |
|------|:--:|:--:|:--:|------|------|------|
| 普通用户执行 | 1000 | 1000 | 1000 | 以 1000 运行 | 报错退出 | 以 1000 运行 |
| `./bash -p` 后 | 1000 | 0 | 0 | setuid(1000) 降权 | 保持 euid=0 | 保持 euid=0 |
| `./a` setuid(0) 后 | 0 | 0 | 0 | ruid==0 → 跳过 | 跳过 | 跳过 |
| root 直接执行 | 0 | 0 | 0 | ruid==0 → 跳过 | 跳过 | 跳过 |

### 1.7 回答三个问题

**问题 1**：`id` 被编译期声明为 `BB_SUID_DROP`。`check_suid()` 在 `id_main()` 之前执行 `setuid(1000)`，euid 和 suid 均被降为 1000。`id_main()` 内部 `geteuid() == getuid()`，因此不额外打印 euid 行。

**问题 2**：`bash -p` 的进程 euid=0，拥有 `CAP_SETUID`。从特权状态调用 `setuid(0)` 时，Linux 内核将 ruid、euid、suid 全部设为指定值（0）。这是标准的 Linux UID 行为。

**问题 3**：取决于 applet 编译期声明的模式——DROP 类主动降权为 ruid；REQUIRE 类保持 euid=0（但校验不通过会直接拒绝执行）；MAYBE 类不降权。
...
使用不经过 BusyBox applet 入口的程序，避免触发 `check_suid()`：

查看非busybox链接文件的工具

```bash
ls -alh /usr/bin | grep -Ev '/bin/busybox'
```

如果有sudo，比较方便，另外alpine的more默认是busybox的链接文件，如果是`util-linux`包里的GNU more，可以拿来用，setpriv是`setpriv`包里的工具，Alpine 3.20+默认安装了

```bash
sudo -i
more /root/root.txt                    # 读取 root 文件
setpriv --reuid=0 --regid=0 --clear-groups /bin/bash  # 恢复完整 root
```
...
### 3.4 sanitize_env_if_suid() — 环境变量清理
