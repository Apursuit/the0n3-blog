...
sidebar: true
description: >-
  Alpine Linux 提权踩坑分析：SUID Bash 拿到 euid=0 的 shell 后，为什么 id、cat 等命令仍显示普通用户？根源是 BusyBox 的 check_suid() 在 ruid≠0 时按 applet 策略主动降权。本文结合 BusyBox 源码与实验拆解该机制，并给出输出重定向、编译 setuid(0) 程序、改用非 BusyBox 独立工具三种绕过方式，附 Docker 最小复现环境。
---
