...
sidebar: true
description: >-
  SUID / sudo 提权技巧收集：三种把普通命令变成提权原语的方法——sudo apk 借助恶意包 post-install 脚本以 root 执行命令、sudo tree --fromfile 任意文件写入（可改 sudoers、authorized_keys）、suid touch 配合 umask 000 写 ld.so.preload 加载恶意 SO。含各方案的适用发行版（glibc / musl）与复现命令。
---
