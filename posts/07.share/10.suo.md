...
description: >-
  MazeSec suo 靶机设计流程：记录靶机构建过程——修改主机名与创建用户、用 PHP + Twig 搭建带 SSTI 漏洞的「代理配置生成器」、安装 arp-scan、配置 apache 与 lanyangyang 的 sudo 授权、放置 flag，并用 iptables 限制出网（仅放行 DNS 与 80），还原一个受限的出题环境。
---
