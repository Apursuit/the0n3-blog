...
description: >-
  rce-labs 命令注入靶场 Writeup：从文件读取命令（cat/tac/head/tail/less/od/base64 等）、外带数据（ping/curl/nc）、写 WebShell（echo/wget），到各类绕过技巧——通配符、变量拼接、命令分隔符（;/&&/||）、命令替换、注释与空格绕过（$IFS、Tab、{}）、引号与黑名单绕过，以及 PHP Session 文件包含，附 Docker 部署方式。
---
...
```bash
git clone https://gh-proxy.org/https://github.com/Apursuit/rce-labs.git
cd rce-labs/docker
docker-compose up -d
```
...
hexdump 以十六进制和 ASCII 格式输出文件内容，默认以十六进制和 ASCII 格式输出
