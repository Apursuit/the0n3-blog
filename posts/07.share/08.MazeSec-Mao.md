...
在第一次设计靶机时，你可能会很兴奋，迫不及待想把想法实现出来；也可能只有零散思路，在信息收集、漏洞利用、GetShell、横向或提权等环节上尚未完全理清。这些都是正常的，也是必经过程。建议先把已有的想法记录下来：可以是一个攻击链、一段思路、一个功能点，或是你希望选手学到的知识。无需一开始就追求完整和完美，先动起来，再逐步完善。
...
如果你已经看到这里，说明靶机已经基本构建完成，接下来进入测试阶段。

测试同样是非常关键的一环。建议在本地环境完整走一遍攻击流程，站在选手的角度尝试攻克这台靶机，重点关注：
...
我们有测试团队来进行最终的测试，但也请在交付前先自行完成一轮本地测试，确保整体流程可正常运行，从而减少沟通与反复修改的成本。感谢配合！
