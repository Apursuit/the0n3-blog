...
description: >-
  MazeSec 矛计划靶机制作指南：以 Search 靶机为例，完整演示从模板 OVA 导入、用户与密码配置、Apache + PHP + Feehi CMS 部署、MySQL 5.7 二进制安装、sudo 与 SSH 密钥配置、flag 放置，到擦除痕迹、导出 OVA、完整测试的靶机出题全流程，并总结出题规范、dirsearch 参数挖掘提权思路与踩坑记录。
---
...
建议先把已有的想法记录下来：可以是一个攻击链、一段思路、一个功能点，或是你希望选手学到的知识。无需一开始就追求完整和完美，先动起来，再逐步完善。

在开始构建靶机前，请先明确：一台合格的靶机，首先应该是可推理、可验证、可复现的。

出题时建议先想清楚三件事：

- 这台靶机希望选手学到什么
- 这条攻击链的核心知识点是什么
- 选手能否仅依靠信息收集与逻辑推理，逐步走到预期路径

靶机可以有难点，但逻辑不能断。选手可以因为不会相关知识点而卡住，但不应该因为缺少线索、只能猜文件名、猜路径、猜思路而卡住。预期攻击链中的关键步骤，也应尽量有可被发现的依据，例如指纹信息、目录结果、报错信息、配置痕迹、权限特征或程序行为。

测试时，除了验证“是否能打通”，也建议反过来检查：这道题是否存在“知道答案后觉得简单，不知道答案时毫无方向”的情况。如果有，这台靶机大概率还需要继续打磨。

准备构建前，请务必务必阅读 MazeSec 靶机出题规范，并在设计阶段优先保证题目的可推理性、可验证性与教学价值，避免因为一些过于隐晦的提示成为解题的关键。

[https://dcn8vd5zcs4n.feishu.cn/wiki/JFUowzkZciAsjokYGBBcXQjhnKf](https://dcn8vd5zcs4n.feishu.cn/wiki/JFUowzkZciAsjokYGBBcXQjhnKf)

大多数靶机，并不是一开始就设计完整的，而是在不断补充与推翻中逐渐成型的。

靶机构建完成后，请务必在本地环境完整测试一遍，建议作者本人完整走一遍攻击流程，站在选手的角度尝试攻克这台靶机，重点关注：
...
<p style='color:red'>我们有测试团队来进行最终的测试，但也请在交付前先自行完成一轮本地测试，确保整体流程可正常运行，不要在测试人员测试阶段出现低级错误</p>以下低级错误可能会导致测试人员无法正常测试：
...
- 给出的凭据无法登录
- 关键文件权限错误导致无法利用
- 个人特定亚文化知识（动漫、游戏、音乐、文学等）背景成为解题关键，此类设计不可视为OSINT线索

若靶机同时存在以上多个问题，反复浪费测试人员时间和精力，我们将拒收靶机。



好了，下面是 Search 这台靶机的构建流程示例，欢迎参考。
...
![](/images/maze-mao/3.png)

导入后，可能需要调整到物理网卡桥接模式，校园网的话，可能限制设备数量，可以选择仅主机模式。

![](/images/maze-mao/4.png)

![](/images/maze-mao/5.png)
...
> -m 表示创建用户的同时创建用户主目录， -s 指定用户登录后使用的 shell

```bash
useradd -m -s /bin/bash 7r1umphk
```
...
![](/images/maze-mao/28.png)

随便想一个admin用户密码，例如：MazeSec2025

随便想一个邮箱，admin@maze-sec.com

![](/images/maze-mao/29.png)

![](/images/maze-mao/30.png)
...
![](/images/maze-mao/31.png)

![](/images/maze-mao/32.png)
...
![](/images/maze-mao/41.png)


指定靶机

![](/images/maze-mao/42.png)


导出路径选择

![](/images/maze-mao/43.png)

导出后，可以把这个靶机ova文件分发给选手

![](/images/maze-mao/44.png)
...
![](/images/maze-mao/52.png)

![](/images/maze-mao/53.png)
...
![](/images/maze-mao/1.png)

![](/images/maze-mao/2.png)
