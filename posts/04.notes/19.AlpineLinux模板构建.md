...
description: >-
  Alpine Linux 靶机模板构建指南：从 VirtualBox 安装 Alpine、用 net.ifnames=0 解决 VirtualBox/VMware 网卡命名不一致，到定制 SSH 与 tty 登录提示、安装 sudo、搭建 nginx + PHP-FPM Web 环境，并汇总 apk、OpenRC 服务、sudo/SUID、cron、用户管理与痕迹清理等常用操作，适合制作轻量靶机与漏洞演示模板。
---
