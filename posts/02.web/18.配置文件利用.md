...
description: >-
  文件上传之 .user.ini 与 .htaccess 配置文件利用：讲解 .user.ini 的 auto_prepend_file / auto_append_file 如何影响所在目录及子目录的 PHP（配合图片马包含），以及 .htaccess 的 AddType / AddHandler / SetHandler / php_value 把图片当 PHP 解析、配合伪协议包含，并附常见 WAF 的内容填充绕过思路。
---

## .user.ini

.user.ini于2009年php5.3版本首次引入[Added ".htaccess" style user-defined php.ini files support for CGI/FastCGI - PHP](https://www.php.net/ChangeLog-5.php#PHP_5_3)
...
如果web服务器是apache，此时 .htaccess 配置文件可以正常解析，如果目录下同时存在php的配置文件 .user.ini ，那么对于同一个配置项，是否存在优先级？（26/07/14留）
