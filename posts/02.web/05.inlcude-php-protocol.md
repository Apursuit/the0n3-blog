...
description: >-
  PHP 文件包含漏洞利用合集：从 include/require 基础到本地与远程文件包含，涵盖日志包含、session 文件包含（条件竞争脚本）、pear 文件包含（pearcmd config-create 写马）、RFI，以及 php://filter、data://、php://input 等伪协议读文件与 GetShell 的 payload。
---
