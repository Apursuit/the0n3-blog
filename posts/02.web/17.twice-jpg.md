...
description: >-
  文件上传二次渲染绕过：针对服务器对图片二次渲染（imagecreatefrompng/jpeg 后重新输出）仍要保留木马的需求，给出 PNG 直接生成可用图片马的脚本，以及 JPG 二次渲染流程（先上传原图、保存渲染图、再用脚本注入 payload），并附 Python 脚本在图片尾部追加随机串爆破出可用的二次渲染图。
---
...
![png](/images/show-upload/png.png)
...
![png1](/images/show-upload/png1.png)
...
![wolf](/images/show-upload/wolf.png)
