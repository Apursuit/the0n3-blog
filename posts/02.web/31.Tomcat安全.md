...
description: >-
  Tomcat PUT 任意文件上传漏洞（CVE-2017-12615）复现：当 conf/web.xml 的 readonly 被设为 false 时，PUT 请求可向 webapps/ROOT 写入任意文件。文章演示 Windows（Tomcat 7.0.79）与 Linux（vulhub + Tomcat 8.5.19）两套环境，以及 /1.jsp/、空格、::$DATA、大小写、末尾点等 JSP 绕过姿势。
---
...
![1](/images/tomcat-put/1.png)

![2](/images/tomcat-put/2.png)
...
![13](/images/tomcat-put/13.png)


文件名后添加`::$DATA`，表示指定流，

![14](/images/tomcat-put/14.png)

**windows特性：文件名大小写不敏感绕过**

windows系统文件名大小写不敏感，可以绕过tomcat的检测

![15](/images/tomcat-put/15.png)

 <p style="color:red">上传是上传成功了，但是不解析大写的JSP</p>

**windows特性：自动去除末尾多出的点**

在文件名后添加`.`，windows系统会自动去除多余的点

![16](/images/tomcat-put/16.png)
