...
description: >-
  ctfshow web126 Writeup：分析为何超出长度限制的 payload 仍能执行——题目用 strlen 限制 fun 参数长度，但 strlen 被 disable_functions 禁用后返回 NULL，NULL < 16 恒为 true，长度限制失效。附带 PHP 非法变量名绕过（CTF[SHOW.COM）、phpinfo 替代函数与 error_reporting 调试技巧。
---
