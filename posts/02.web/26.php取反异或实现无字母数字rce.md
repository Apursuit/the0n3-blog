...
description: >-
  PHP 无字母数字 RCE：用取反（~）、异或（^）、或（|）三种位运算把函数名和参数构造成不可见字符 payload，绕过字母数字过滤，并给出三种运算的 payload 生成脚本（基于动态调用 ('函数名')('参数')）。
---
