...
description: >-
  使用 Weblog 平台接收无回显 RCE 命令执行结果：针对 exec / shell_exec / 反引号等无回显的命令执行，讲解写文件、curl + DNSLog 外带、反弹 shell 等思路；并说明 DNSLog 遇空格截断的缺陷，改用 Weblog 平台通过 POST 请求外带，拿到完整的命令执行结果（以 ctfshow web807 为例）。
---
...
举例，以 ctfshow 题目[web807](https://ctf.show/challenges#web807-1847)举例：

题目这里shell_exec会对于用户传入的url进行访问，没有任何过滤，可以在shell_exec函数里使用分号执行多个命令

![1](/images/rce-weblog/1.png)
