...
description: >-
  PHP Session 文件包含与条件竞争利用：利用 session.upload_progress 在上传时把可控内容写入 /tmp/sess_<PHPSESSID>，再通过文件包含点执行；针对 cleanup=On 的情况用多线程条件竞争在 session 被清空前包含，附 POST / GET 两种表单的完整 Python 脚本与状态码排错。
---
...
![1](/images/session-include/1.png)
...
![1](/images/session-include/2.png)
...
![QA](/images/session-include/qa.png)
...
![4](/images/session-include/4.png)
...
![3](/images/session-include/3.png)
...
![5](/images/session-include/5.png)
...
状态码500的补救措施 ： 修改脚本中的sessionid，shell文件名，接着跑。创建一个新的shell
