...
description: >-
  浅析 PHAR 反序列化：讲解 phar 文件结构（stub / manifest / contents / signature）与 meta-data 反序列化触发点，说明利用前提（存在文件上传/读写 + 文件操作函数），并给出生成 phar 文件、通过 file_get_contents 等函数触发反序列化的完整示例，以及扩展名、文件头、伪协议等绕过方式。
---
...
![3](/images/flask-shell/3.png)

![4](/images/flask-shell/4.png)
