...
description: >-
  MD5 相关绕过技巧汇总：数组绕过 md5/sha1 强比较、0e 弱比较（240610708 等 payload）、$md5=md5($md5)、fastcoll 构造相同 MD5、md5($pwd,true) 引发 SQL 注入（ffifdyop），以及 MD5 长度扩展攻击的原理与脚本（以 BaseCTF 题目为例）。
---
