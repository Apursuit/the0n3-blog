...
description: >-
  Flask debug console PIN 码计算：讲解 Werkzeug PIN 的 probably_public_bits（username、modname、appname、moddir）与 private_bits（网卡 MAC 转十进制、machine-id 由 /etc/machine-id、boot_id、/proc/self/cgroup 拼接）的获取方式，并给出 MD5（低版本）与 SHA1（高版本）两套计算脚本。
---
...
![1](/images/flask-PIN/1.png)

控制台也可以正常访问

![2](/images/flask-PIN/2.png)
