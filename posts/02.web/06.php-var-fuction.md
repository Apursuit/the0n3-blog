...
description: >-
  PHP 无参 RCE（仅用函数实现命令执行与读文件）：借助 get_defined_vars / next / array_pop / eval 构造无参命令执行 payload，以及用 getcwd / localeconv / scandir / array_reverse / next / show_source 链式读取当前目录文件源码，源自 ctfshow web40。
---
