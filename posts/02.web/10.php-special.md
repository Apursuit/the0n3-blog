...
description: >-
  CTF PHP 特性利用总结：涵盖数组绕过 preg_match / intval、弱比较（== 与 ===）、字符串与数字比较、intval 进制绕过、md5/sha1 的数组绕过与 0e 弱比较绕过、md5($pwd, true) SQL 注入、fastcoll 构造相同 MD5 文件，以及 parse_str、strrev、ereg 等函数特性。
---
