...
description: >-
  PHP 远程命令执行（RCE）绕过技巧合集：系统整理通配符、命令逃逸、反引号回显、变量拼接、空格绕过（%09 / %0a / ${IFS} / 重定向 / {}）、转义与单双引号绕过、八进制、编码管道，以及无字母数字 RCE 的取反、自增、异或构造等方法，附构造脚本与 payload。
---
...
![3](/images/ctfshow_web30_命令执行/3.png)
...
![2](/images/ctfshow_web30_命令执行/2.png)
