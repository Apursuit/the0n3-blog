...
补充下linux基础知识：/proc 是 Linux 内核提供的虚拟文件系统，用于在运行时向用户空间暴露内核和进程信息，其中的 /proc/<PID>/cmdline 记录进程启动时的完整命令行参数，/proc/<PID>/environ 保存进程启动时的环境变量快照，二者都是内核实时生成的只读数据，广泛用于进程排查、安全审计和系统调试。
