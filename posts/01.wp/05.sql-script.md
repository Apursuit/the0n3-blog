...
description: >-
  CTFshow SQL 注入 web171-253 通关 wp：覆盖字符型与数字型注入、联合查询、报错注入、布尔与时间盲注，以及宽字节、堆叠、limit / update / insert / delete 注入、UDF 与 NoSQL 等大量关卡与 payload。
---

## 前置知识

> 这一篇是 ctfshow「web 入门」SQL 注入 171~253 的刷题记录，共 80 多题。写的时候默认读者已经会 SQL 注入基础，所以先补上几块地基，零基础也能顺着往下看。

### 1. 什么是 SQL 注入

后端把用户输入直接拼进 SQL 语句执行，没有过滤。比如：

```php
$sql = "select username,password from user where id = '".$_GET['id']."'";
```

你传的 `id` 会原样进到 SQL 里。正常传 `1`，语句是 `... where id = '1'`；如果传 `1' or '1'='1`，语句就变成了 `... where id = '1' or '1'='1'`——多出来的 `or '1'='1'` 是你自己加的 SQL，这就是注入。

### 2. 数字型 vs 字符型

看题目给的 SQL 里，注入点有没有被引号包着：

- **数字型**：`where id = {id}`（没引号）→ 直接拼接，如 `id = 1 or 1=1`
- **字符型**：`where id = '{id}'`（有引号）→ 要先闭合引号，如 `id = 1' or '1'='1`

本文大部分是字符型，所以 payload 里总能看到开头那个 `'`——它的作用就是"先把题目写死的那个引号闭合掉"。

### 3. 联合查询 union select

`UNION` 能把两个 `SELECT` 的结果拼在一起。注入时用它最关键的两条规则：

1. **列数必须一致**：原查询 `select` 了几列，你的 `union select` 就得跟几列，否则报错。所以常见 payload 里那串 `1,2,3` 是"占位符"，用来凑够列数。
2. **让原查询没结果**：在 id 前加 `-1`，让 `id = '-1'` 查不到任何行，这样页面上显示的就只剩你 union 出来的结果。

### 4. 注释符

注入时要"掐掉" SQL 语句后半段（比如题目写死的 `limit 1` 或多余的引号），用注释：

- `#`（在 URL 里常写成 `%23`）
- `-- `（两个减号加空格，也常写成 `--+`）

### 5. information_schema（盲注的核心）

MySQL 自带一个 `information_schema` 库，记录着所有库、表、字段的"元数据"。盲注爆数据全靠它：

| 表 | 作用 |
|---|---|
| `information_schema.schemata` | 所有数据库名 |
| `information_schema.tables` | 所有表名（`table_schema=database()` 限定当前库） |
| `information_schema.columns` | 所有字段名（`table_name='xxx'` 限定某表） |

### 6. 盲注是什么

当页面**没有直接回显**查询结果时，不能一眼看到数据，只能靠"是/否"或"时间差"一点点猜，这就是盲注。三种：

- **布尔盲注**：看返回结果"是/否"变化（如是否出现某关键字）。
- **时间盲注**：看响应快慢（`if(条件,sleep(3),0)`，成立就卡 3 秒）。
- **报错注入**：故意让 SQL 报错，把数据塞进报错信息里带出来。

### 7. 本文结构（按考点分组）

| 题号 | 考点 |
|---|---|
| 171~174 | 基础：闭合、联合查询、盲注入门 |
| 175~200 | WAF 绕过：空格/关键字/函数过滤、堆叠、大小写 |
| 201~213 | sqlmap 自动化 |
| 214~220 | 时间盲注 |
| 221~230 | limit、group by、报错、finfo |
| 231~243 | update / insert / delete / 写文件 |
| 244~248 | 报错注入、UDF 提权 |
| 249~253 | NoSQL（Memcache / MongoDB） |

## 函数速查表

后面大量 payload 会用到这些 MySQL 函数，这里集中列一下，遇到忘了回来查。

### 字符串截取（盲注逐位猜的核心）

| 函数 | 作用 | 示例 |
|---|---|---|
| `substr(s,i,len)` / `mid(s,i,len)` | 从第 i 位截 len 个字符 | `substr('abc',2,1)` → `b` |
| `left(s,len)` | 从左截 len 个 | `left('abc',2)` → `ab` |
| `right(s,len)` | 从右截 len 个 | `right('abc',2)` → `bc` |
| `ascii(c)` / `ord(c)` | 字符转 ASCII 数字 | `ascii('a')` → `97` |
| `hex(s)` | 转 16 进制 | `hex('a')` → `61` |
| `concat(a,b,...)` | 拼接字符串 | `concat('a','b')` → `ab` |
| `group_concat(col)` | 把一列多行拼成一行（爆多行数据必用） | `group_concat(table_name)` |
| `replace(s,a,b)` | 把 s 里的 a 替换成 b | `replace('flag','f','F')` |

### 条件判断

| 函数/符号 | 作用 |
|---|---|
| `if(条件,a,b)` | 条件成立返回 a，否则 b（盲注核心） |
| `or` / `\|\|` | 逻辑或（`\|\|` 是 or 的等价写法，用于绕过过滤） |
| `and` / `&&` | 逻辑与 |
...
| 函数 | 作用 | 示例 |
|---|---|---|
| `like` | 模糊匹配，`%` 匹配任意多字符 | `username like 'f%'` |
| `regexp` / `rlike` | 正则匹配 | `pass regexp '^ctfshow'` |

### 报错注入函数（让 SQL 报错并带出数据）

| 函数 | 说明 |
|---|---|
| `updatexml(1,concat(0x7e,数据,0x7e),1)` | xpath 格式错误时报错，报错信息里带出数据 |
| `extractvalue(1,concat(0x7e,数据,0x7e))` | 同上 |
| `floor(rand(0)*2)` + `group by` | 利用 group by 插入冲突报错 |
...
|---|---|
| `sleep(n)` | 暂停 n 秒 |
| `benchmark(n,expr)` | 把 expr 执行 n 次（耗时，代替 sleep 绕过过滤） |
...
注入点在 `id`，而且 `id` 被单引号包着，是**字符型注入**；没有做任何过滤。

先看它想干嘛：`username !='flag'` 意思是"不查 username 为 flag 的那一行"——说明 flag 就藏在 `username='flag'` 那一行的 `password` 字段里，题目的过滤是不让你直接查它。
...
1. 传进去后语句变成 `... where username !='flag' and id = '1' or '1'='1' limit 1;`
2. `1'` 里的 `'` 把题目写死的 `id = '` 那个引号**闭合**了；
3. `or '1'='1'` 是你注入的条件，`'1'='1'` 永远成立；
4. 整条 `where` 因为 `or` 恒真，就绕过了 `id = ...` 的限制。

这种"让条件恒真、绕开查询/登录限制"的 payload 就叫**万能密码**。不过这道题还有 `username !='flag'` 挡着，万能密码拿到的是普通用户的数据，要拿 flag 得看下一题的联合查询。

## web 172

这题 `username` 被过滤了（不能出现 'flag'），flag 的值在 `password` 字段里。光靠万能密码绕不过 `username !='flag'` 的限制，这里改用**联合查询 union select**。
...
1. `-1'`：`-1` 让 `id = '-1'` 查不到任何行（表里没有 id=-1），`'` 闭合引号。这样原查询返回空，页面上只会显示你 union 出来的结果。
2. `union select 1,password`：联合查询。原查询 `select username,password` 是**两列**，所以这里也必须两列——`1` 是占位符（占 username 那列），`password` 是真正想查的列。
3. `from ctfshow_user2`：从这张表查，flag 就在它的 password 列。
4. `%23`：URL 编码的 `#`，注释符，把后面题目写死的 `limit 1` 注释掉。

> 记住这条规律：**union select 的列数必须和原查询一致，不够就用 1,2,3 占位**。后面所有联合查询题都是这个套路。

## web 173

这题查询结果里不能出现 flag（有检测），而 flag 是 `username` 字段里的值，所以不能查 username 列。另外注意：这题原查询是**三个字段**（三列）。
...
和上题对比，变化点就在**占位符从两个变成三个**：因为原查询 select 了三列，union 也得跟三列，所以用 `1,2` 占前两列、`password` 查第三列。不查 username 列，就绕开了"结果不能有 flag"的检测。

> 核心就一句：**列数变了，占位符跟着变**。看到题目 SQL 里 select 几个字段，你的 union 就凑几个值。

## web 174

这题开始进入**盲注**：结果里过滤了数字（不能直接看到数据），得靠"逐位比较"把 flag 一位位猜出来。给了两个方法。

### 方法 1：布尔盲注

思路：flag 的每一位，用 `substr` 截出来，和候选字符逐个比较，猜对了就返回不同结果（这里是页面出现 "admin" 关键字）。

```python
import requests
import string
flag = ''
table = string.digits + string.ascii_letters + '-{}'   # 候选字符集：数字 + 字母 + -{}
for i in range(1, 45):     # i 是第几位，假设 flag 长度 < 45
    for j in table:        # 对每一位，遍历候选字符
        # url去掉s  接口在网络面板里可以看到
        ...
        if "admin" in r.text:   # 猜对了，页面返回里出现 admin
            flag += j
            print(flag)
            break
```

核心就一句：`substr((select password ...), i, 1) = j` —— 取 password 的第 i 位，看它等不等于候选字符 j；等的话整条 `and` 成立、页面返回 admin，就说明第 i 位是 j。这样一位位拼出完整 flag。

### 方法 2：replace 替换数字

这题结果里把数字过滤了（数字会被替换/拦截），所以先用 `hex()` 把数据转成 16 进制，再用 `replace` 把 16 进制里的每个数字替换成 `nba`/`nbb`...这种不含数字的字符串，绕过数字过滤，查出来后再还原。
...
还原脚本（拿到结果后，反向把 nba~nbj 替换回 0~9，再 16 进制转明文）：
```python
...
> 两种方法本质一样：**没回显/有过滤时，把 flag 拆成一位位，用"是/否"（布尔）或"替换编码"的方式间接拿**。这是后面所有盲注题的底子。
...
![scripts](/images/show-sql/script.png)
...
![upload](/images/show-sql/upload.png)
...
![time0](/images/show-sql/time0.png)

抓`/api/`的报文还是什么都没有

![214](/images/show-sql/214.png)
...
![230](/images/show-sql/230.png)

先测试修改ctfshow用户的密码

![2301](/images/show-sql/2301.png)

接口返回信息，修改成功，可以正常执行

![2302](/images/show-sql/2302.png)
...
![232](/images/show-sql/232.png)

![2321](/images/show-sql/2321.png)
...
![237](/images/show-sql/237.png)

在insert.php页面源码里可以看到接口

![2371](/images/show-sql/2371.png)
...
![240](/images/show-sql/240.png)

![2401](/images/show-sql/2401.png)

回到题目界面刷新倒序，看到flag已经插入进去了

![2402](/images/show-sql/2402.png)

## web 241 delete

在调试工具里可以看到删除逻辑的处理请求

![241](/images/show-sql/241.png)
...
![2443](/images/show-sql/2443.png)

## web 245

过滤了`updatexml`函数，还可以用类似功能函数`extractvalue`

![245](/images/show-sql/245.png)
...
![247](/images/show-sql/247.png)

爆金币

![2471](/images/show-sql/2471.png)
...
![248](/images/show-sql/248.png)

## web 249 nosql

只有一句sql语句

![249](/images/show-sql/249.png)
...
![2491](/images/show-sql/2491.png)

传个id数组

![2492](/images/show-sql/2492.png)

## web 250 MangoDB

补充MangoDB相关语法，因为他和一般sql字段，语法不完全相同

![250](/images/show-sql/250.png)
...
![2501](/images/show-sql/2501.png)

看一下登录接口

![2502](/images/show-sql/2502.png)
...
![251](/images/show-sql/251.png)

## web 252

再用上面payload，没有flag

![252](/images/show-sql/252.png)

使用正则匹配password里的flag

![2521](/images/show-sql/2521.png)
...
![253](/images/show-sql/253.png)

![2531](/images/show-sql/2531.png)
