---
title: HackMyVM Realsaga 复盘：WordPress 插件漏洞 + Docker 逃逸
...
description: >-
  HackMyVM Realsaga 靶机复盘：wpscan 爆破出 WordPress 用户，nuclei 发现 easy-wp-smtp 插件漏洞——通过调试日志里的密码重置链接接管管理员 wpadmin；改插件 Getshell 后进入 Docker 容器，用 SUID find 拿到容器 root，再借挂载的 docker.sock（docker.saga）挂载宿主机根目录完成 Docker 逃逸。
---
...
![](/images/hmv-realsaga/3.png)

![](/images/hmv-realsaga/4.png)

日志里可以找到最新的重置密码链接

![](/images/hmv-realsaga/5.png)

![](/images/hmv-realsaga/6.png)

## 获取 Webshell

使用管理员用户 wpadmin 登录 WordPress 后台，修改插件内容，例如 你好多莉 插件，添加一句反弹 shell 命令，保存并在安装的插件中启用你好多莉插件即可拿到 反弹shell

![](/images/hmv-realsaga/7.png)
