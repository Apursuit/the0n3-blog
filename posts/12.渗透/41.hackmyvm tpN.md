...
description: >-
  HackMyVM tpN 靶机 Writeup：8080 的 ThinkPHP 应用存在 .git 泄露，用 git-dumper 下载源码审计出 Admin/hello 后门（call_user_func）与 session 校验逻辑；先访问 /index/token/token 拿合法 session，再用 GET 传参绕过 default_route_pattern 限制反弹 shell，最后用 Dirty Pipe（CVE-2022-0847）覆盖 SUID gpasswd 提权到 root。
---
...
![](/images/hmv-tpn/1.png)

使用 ThinkPHP 综合利用工具扫一圈，没有发现可利用点。

![](/images/hmv-tpn/2.png)

## 目录扫描

使用 dirsearch 进行目录扫描，发现靶机存在 .git 目录泄露，同时扫描到了 phpinfo 页面。

![](/images/hmv-tpn/3.png)

![](/images/hmv-tpn/4.png)

![](/images/hmv-tpn/5.png)
...
![](/images/hmv-tpn/12.png)

可以在配置里的路由规则里找到原因，默认的路由变量只使用正则匹配 `[1-9]`。
![](/images/hmv-tpn/13.png)

在 url 里直接传参 a、b，执行成功。

![](/images/hmv-tpn/14.png)
