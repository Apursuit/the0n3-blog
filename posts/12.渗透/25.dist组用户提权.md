...
description: >-
  Linux disk 组提权笔记：利用 disk 组可读写块设备，用 /usr/sbin/debugfs -w /dev/sda1 直接篡改文件系统——读取 root 私钥与 /etc/shadow、写 sudoers.d、写入 authorized_keys、投放 cron 定时任务、添加 passwd/shadow 后门用户。含 debugfs 引发内核缓存不一致、文件系统只读、虚拟机崩溃等异常分析及推荐提权顺序。
---
