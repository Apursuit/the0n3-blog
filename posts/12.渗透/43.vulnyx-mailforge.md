...
description: >-
  Vulnyx MailForge 靶机 Writeup：5000 端口的 eml 邮件预览服务存在 Flask SSTI，在 Subject 字段注入 {{config}} 确认后用 url_for 全局变量执行命令反弹 shell；再借可写的 sudo 脚本 /usr/local/bin/cleaner.sh 提权到 root，并分析 LinPEAS 对 BusyBox su 的误报原因。
---
...
![](/images/vulnyx-mailforge/4.png)

直接修改内容，进入 root shell。

![](/images/vulnyx-mailforge/5.png)

## linpeas 误报

跑了下 linpeas，发现有 suid 的 doas （可能需要配置文件才能利用）。

![](/images/vulnyx-mailforge/6.png)

LinPEAS 报告显示几乎所有密码都能登录 root，这显然是误报。

![](/images/vulnyx-mailforge/7.png)
...
![](/images/vulnyx-mailforge/8.png)

修复建议：

使用 `$?` 检查 su 命令的退出状态码，不依赖 stdout 内容。su 成功时返回 0，失败时返回非 0。

![](/images/vulnyx-mailforge/9.png)
