...
description: >-
  Vulnyx alpine 靶机 Writeup：Web 目录下的 profile.html 泄露 developer 的 SSH 密码，登录后按 README 提示定位到 sysadmin 的 webapp 仓库，用 git log 从提交记录里找到私钥登录 sysadmin；再发现 /opt/cleaner.sh 由 root 定时执行且 sysadmin 可写，追加 sudoers 无密码配置完成提权。
---
...
![](/images/vulnyx-alpine/1.png)

## 目录扫描

使用 dirsearch 进行目录扫描，发现 /profile.html 页面。

![](/images/vulnyx-alpine/2.png)
...
![](/images/vulnyx-alpine/4.png)

![](/images/vulnyx-alpine/5.png)
...
![](/images/vulnyx-alpine/6.png)

或者跑 pspy64 ，也可以找到这个文件，uid 0 执行，也就是 root 权限执行。

![](/images/vulnyx-alpine/7.png)

![](/images/vulnyx-alpine/8.png)
