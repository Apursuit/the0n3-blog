...
description: >-
  CVE-2020-0796 永恒之黑（SMBGhost）漏洞复现：Windows SMBv3.1.1 处理压缩消息时未校验长度导致整数溢出，可无权限远程执行代码，影响 Win10 1903/1909 及对应 Server Core。含 fscan / 扫描脚本探测、蓝屏 DoS 与 RCE 两种利用（msfvenom 生成 bind shell + SMBGhost_RCE_PoC），以及禁用 SMBv3 压缩、安装 KB4551762 补丁等修复建议。
---

fscan扫描本机，存在永恒之黑漏洞，漏洞复现一下

经典漏洞复现：CVE-2020-0796 SmbGhost Vulnerable
...
![10](/images/永恒之黑/10.png)

这里是exploit.py里的内容

![11](/images/永恒之黑/11.png)
