---
title: Wordpres wp2shell未授权RCE复现
date: 2026-07-20 23:20:55
permalink: /posts/wp-wp2shell/
tags: []
categories: 
- WordPress
draft: false
sidebar: true
description: >-
  WordPress 未授权 RCE 复现（wp2shell）：组合 CVE-2026-63030 的 REST API 批量路由混淆与 CVE-2026-60137 的 WP_Query author__not_in SQL 注入，无需认证即可创建管理员账号，再借插件执行任意命令。基于 vulhub docker-compose 一键搭环境，附 PoC 脚本用法（--check 仅验证盲注）。
---

该漏洞可在vulhub环境通过docker compose快速复现[https://github.com/vulhub/vulhub/tree/master/wordpress](https://github.com/vulhub/vulhub/tree/master/wordpress)
...
CVE-2026-60137 是 WP_Query 中的 SQL 注入漏洞。`author__not_in` 参数只有在以数组形式传入时才会被转换为整数；如果传入字符串，该值便可能被直接拼接进 post_author NOT IN (...) 子句。攻击者可以在一个 REST API 批量请求中嵌套另一个批量请求，并提交 GET `/wp/v2/categories?author_exclude=`<SQL>。分类接口没有注册 `author_exclude` 参数，因此该值能够原样通过校验；路由混淆随后使这个请求由文章接口的 get_items() 处理器执行。该处理器会把 `author_exclude` 映射到存在漏洞的 `author__not_in` 查询变量，从而形成未授权盲注 SQL 注入。

此环境中的漏洞利用方式将 SQL 注入升级为通过 WordPress 文章对象缓存和自定义程序变更集工作流程创建未经授权的管理员。

参考：

- [vulhub WordPress REST API 批量路由混淆与 SQL 注入导致的未授权 RCE（wp2shell，CVE-2026-63030 / CVE-2026-60137）](https://github.com/vulhub/vulhub/blob/master/wordpress/CVE-2026-63030/README.zh-cn.md)
...
准备docker-compose.yml文件[https://github.com/vulhub/vulhub/blob/master/wordpress/CVE-2026-63030/docker-compose.yml](https://github.com/vulhub/vulhub/blob/master/wordpress/CVE-2026-63030/docker-compose.yml)
...
![docker compose启动环境](/images/wp-wp2shell/1.png)

使用vulhub项目提供的poc验证[https://github.com/vulhub/vulhub/blob/master/wordpress/CVE-2026-63030/poc.py](https://github.com/vulhub/vulhub/blob/master/wordpress/CVE-2026-63030/poc.py)
...
使用`--check`参数可以只验证是否存在sql盲注漏洞

```bash
python poc.py http://192.168.1.4:8080 --check
```

![验证存在wp2shell漏洞](/images/wp-wp2shell/2.png)
...
![poc创建管理员账户](/images/wp-wp2shell/3.png)

使用该账号可以成功登录，并且是管理员权限

![wordpress管理员登录成功](/images/wp-wp2shell/4.png)

可以修改wordpress自带的`hello dolly`插件，执行任意命令

![修改hello dolly插件](/images/wp-wp2shell/5.png)

在插件源码中添加php代码后， 启用该插件，访问插件页面即可执行php代码

![启用插件](/images/wp-wp2shell/6.png)

![wordpress代码执行](/images/wp-wp2shell/7.png)