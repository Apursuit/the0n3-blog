...
> Kerberos 默认要求客户端和域控（KDC）的时间差不能超过 5 分钟（300 秒），使用 时间戳​ 防止重放攻击，超出就会直接拒绝 AS-REQ
...
在 Active Directory 中，拥有对 MSA_HEALTH$ 这类机器账号或托管账号的 GenericWrite 权限，最常见的利用方式是 Shadow Credentials（影子凭据）​ 攻击，即向该对象的 msDS-KeyCredentialLink 属性写入自定义的密钥（该属性存储 X.509 证书的公钥），从而获取该账号的认证票据（TGT），实现提权或伪装该服务账号

原理说明：

- 在 Active Directory 中，用户和计算机对象都有一个名为 msDS-KeyCredentialLink 的属性，它可以存储原始的公钥信息。拥有修改此属性权限的账户（如域管理员，或拥有 GenericWrite 权限的账户），就可以为目标对象添加一个受自己控制的公钥

- Kerberos 认证的第一步是“预认证”（Pre-authentication）。除了使用密码（对称加密），还可以通过证书进行非对称方式的预认证，这就是 PKINIT 协议。客户端用自己的私钥加密数据，而 KDC（域控）用该客户端的公钥解密验证
...
- 权限维持：有了 TGT，攻击者就可以获取目标账户的 NTLM Hash 或进行票据传递（Pass-the-Ticket），实现完全接管。即使目标用户后续更改了密码，植入的 Shadow Credentials 依然有效。
...
WSUS 使用 HTTPS，靶机的 WSUS 客户端配置要求使用 HTTPS (端口 8531) 进行通信，客户端会验证服务器证书是否由受信任 CA 签发，且 CN/SAN 是否为 wsus.logging.htb，没有合法证书 ， 客户端会拒绝连接
...
客户端定期向 /ClientWebService/client.asmx 发送 GetSoftwareUpdates 请求，wsuks 伪造响应，返回恶意 "更新" ， 客户端执行 wuauclt /detectnow 或自动安装，-c 参数中指定的命令会被包装成更新安装脚本执行
