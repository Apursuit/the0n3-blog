...
sidebar: true
description: >-
  HTB Logging 靶机 Writeup（Windows AD，Medium）：审计 SMB 日志共享拿到 svc_recovery 新密码，因账户属 Protected Users 组无法 NTLM 认证，改用 Kerberos 取 TGT；BloodHound 发现对 msa_health$ 有写权限，用 Shadow Credentials（影子凭据）获取其哈希并登录 WinRM；再经 UpdateMonitor 的 DLL 劫持与 WSUS 劫持（AD CS 申请证书 + wsuks）拿到域管理员。
---
...
关键词：Kerberos认证绕过NTLM限制、影子凭据攻击、DLL劫持、WSUS 劫持
...
测试smb、ldap，返回`STATUS_ACCOUNT_RESTRICTION`，账户被限制登录，svc_recovery 属于 Protected Users 组。该组成员禁止使用 NTLM 进行网络认证，因此使用正确密码通过 SMB/WinRM 进行 NTLM 认证时会返回 STATUS_ACCOUNT_RESTRICTION。此时应改用 Kerberos 获取 TGT
...
> Kerberos 默认允许客户端与 KDC 的时钟偏差约为 5 分钟。Kerberos 认证消息包含时间信息，KDC 会检查时间偏差，超过允许范围后通常会返回 KRB_AP_ERR_SKEW

HTB 靶机通常不会提供可用的 NTP 服务，因此无法通过标准 NTP 同步时间，可以通过 SMB2 negotiate 返回的服务器时间手动校准本地时间
...
sudo date -s "2026-07-31 11:06:39"
```
...
nxc使用kerberos 票据做ldap查询列出域中的用户以及计算机/服务账户
...
- 在 Active Directory 中，用户和计算机对象都有一个名为 msDS-KeyCredentialLink 的属性，它可以存储原始的公钥信息。如果攻击者对目标对象具有能够修改 msDS-KeyCredentialLink 的权限，例如适当的 GenericWrite / GenericAll 权限，则可以尝试利用 Shadow Credentials

- Kerberos 认证的第一步是“预认证”（Pre-authentication）。除了使用密码（对称加密），还可以通过证书进行非对称方式的预认证，这就是 PKINIT 协议。在 PKINIT 认证过程中，客户端利用私钥对认证数据进行签名/证明密钥持有，KDC 根据目标账户 msDS-KeyCredentialLink 中保存的公钥验证该证明。如果验证成功，KDC 可以向该账户颁发 TGT
...
公钥 → 写入目标账户 msDS-KeyCredentialLink
...
- 植入公钥：在目标账户的 msDS-KeyCredentialLink 中添加一个由攻击者自己生成的公钥。
- 获取证书：使用这个公钥和对应的私钥，生成一个 X.509 证书。
- 请求 TGT：利用生成的证书，通过 PKINIT 协议向域控发起 Kerberos 认证请求。因为域控在目标账户的属性里找到了匹配的公钥，认证通过，攻击者成功获得该账户的 TGT（Ticket Granting Ticket）。
- 权限维持：在获得目标账户的 TGT 后，可以进一步利用 Kerberos 的相关认证流程获取该账户的 NT hash；Certipy 的 shadow auto 可以自动完成这一过程。即使目标用户后续更改了密码，植入的 Shadow Credentials 依然有效，除非管理员删除对应的 Key Credential，或者采取其他措施使该凭据失效。
...
使用msf生成反弹shell DLL，放到C:\ProgramData\UpdateMonitor\Settings_Update.zip，触发计划任务执行。对于本次利用而言，不需要导出 PreUpdateCheck。因为 LoadLibrary() 成功加载 DLL 时会执行 DLL 的初始化代码，包括 DllMain；因此 Metasploit 生成的 DLL 可以在 DllMain 中触发 payload。随后程序对 PreUpdateCheck 的 GetProcAddress() 查找可能失败，但此时 payload 已经执行
...
由于 WSUS 客户端通过 HTTPS 访问 wsus.logging.htb:8531，攻击者不仅需要将该主机名解析到自己的服务器，还需要提供一个目标系统信任的 CA 签发、且 SAN 包含 wsus.logging.htb 的服务器证书
...
wsuks 构造恶意 WSUS 更新响应，并将指定的 payload/命令封装进更新内容。当 Windows Update 客户端接受并处理该更新后，payload 会以 Windows Update 服务相关的高权限上下文执行
