...
sidebar: true
description: >-
  HTB Support 靶机 Writeup（Windows AD）：从 SMB 空会话 / Guest 枚举发现 support-tools 共享，反编译 .NET 程序 UserInfo.exe 解出 ldap 凭证；LDAP 枚举在 support 用户的 info 字段拿到明文密码，BloodHound 发现 Shared Support Accounts 组对 DC$ 有 GenericAll，最终利用 RBCD（S4U2Self + S4U2Proxy）获取 Administrator 的 CIFS 票据，经 smbexec 拿到 SYSTEM。
---
