...
permalink: /pages/qwb2019-suibianzhu/
sidebar: false
categories:
  - 强网杯
  - 堆叠注入
...
description: >-
  强网杯 2019 随便注 Writeup（BUUCTF）：在过滤 select/update/delete/where 等关键字的情况下，给出三种解法——堆叠注入配合 rename/alter 改表名与字段，再借万能密码回显 flag；16 进制编码 SQL 通过 prepare/execute 预处理执行；以及用 MySQL HANDLER 语句直接逐行读取表数据。
---
...
![sql](/images/daily/sql-test/sql.png)
...
![handler](/images/daily/sql-test/handler.png)