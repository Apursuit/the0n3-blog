...
description: >-
  HCTF 2018 WarmUp Writeup：一道 PHP 文件包含（LFI）题。审计 source.php 的白名单校验，利用 mb_strpos 只截取 `?` 之前内容、以及 urldecode 二次检查的特性绕过白名单，再借 include($_REQUEST['file']) 用 `hint.php?/../../../../` 目录穿越包含 flag 文件。
---
...
![warmup](/images/daily/warmup/1.png)
