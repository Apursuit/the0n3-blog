...
description: >-
  MazeSec ezpwn 靶机 Writeup：栈溢出（ret2text）、栈对齐修复、sudo file GTFOBins 滥用、SSH 私钥提取。选手通过端口扫描发现 9999 端口运行存在 gets 缓冲区溢出的 ELF 程序，利用输入字符位置校验逻辑直接触发后门函数，暴露出内部 11450 端口提供的 ret2text 题目附件；在 ret2text 程序中，通过计算偏移构造 payload 覆盖返回地址，并插入 ret 指令修复栈对齐问题，成功获取 a 用户权限；随后利用 a 用户拥有的 NOPASSWD sudo 权限执行 /usr/bin/file，参照 GTFOBins 利用方式读取 /root/.ssh/id_rsa 私钥，经格式处理后实现免密登录 root，完成权限提升。该靶机重点考察了基础栈溢出利用、栈对齐原理以及 sudo 二进制程序配置缺陷的实战利用。
---


## 靶机说明

**关键词**：栈溢出（ret2text）、栈对齐修复、sudo file GTFOBins 滥用、SSH 私钥提取

**摘要**：本文记录了 MazeSec ezpwn 靶机的渗透过程。选手通过端口扫描发现 9999 端口运行存在 gets 缓冲区溢出的 ELF 程序，利用输入字符位置校验逻辑直接触发后门函数，暴露出内部 11450 端口提供的 ret2text 题目附件；在 ret2text 程序中，通过计算偏移构造 payload 覆盖返回地址，并插入 ret 指令修复栈对齐问题，成功获取 a 用户权限；随后利用 a 用户拥有的 NOPASSWD sudo 权限执行 /usr/bin/file，参照 GTFOBins 利用方式读取 /root/.ssh/id_rsa 私钥，经格式处理后实现免密登录 root，完成权限提升。该靶机重点考察了基础栈溢出利用、栈对齐原理以及 sudo 二进制程序配置缺陷的实战利用。
...
![](/images/maze-ezpwn/2.png)

init函数

![](/images/maze-ezpwn/4.png)

start函数，调用了gets函数（事故多发点），符合条件时会调用port函数

![](/images/maze-ezpwn/3.png)


port函数，调用了system函数，执行了一个port相关脚本`/home/a/overflow/port.sh`，同时说明了存在用户a

![](/images/maze-ezpwn/5.png) 

没有保护措施

![](/images/maze-ezpwn/6.png)
...
![](/images/maze-ezpwn/9.png)

提示，密码的位置提示了(offset)，ssh密码可能是偏移值

![](/images/maze-ezpwn/10.png)


start函数里有gets函数

![](/images/maze-ezpwn/11.png)

程序没有保护

![](/images/maze-ezpwn/13.png)


存在一个后门 shell 函数

![](/images/maze-ezpwn/12.png)

shell 函数地址

![](/images/maze-ezpwn/14.png)
...
![](/images/maze-ezpwn/21.png)

ssh 到root

![](/images/maze-ezpwn/22.png)
