...
description: >-
  MazeSec XIYI 靶机 Writeup：SSRF 链式利用、Gopher 协议打内网、Flask SSTI 反弹 Shell、零宽字符隐写、sudo ln 路径穿越提权。选手通过 80 端口的 SSRF 漏洞，先后利用 dict 协议探测内网端口、tftp 协议读取源码，发现 2332 端口的中继服务和 2333 端口存在 Jinja2 SSTI 的 Flask 应用；借助 Gopher 协议构造恶意 POST 请求穿透 SSRF 边界，在 2333 端口触发模板注入并反弹 Shell，获取 www-data 权限；在 Web 目录中发现包含大量零宽字符的秘密文件，通过十六进制分析与二进制解码提取出 lemon 用户的凭证，成功登录 SSH；最后利用 lemon 的 sudo 权限（NOPASSWD: /usr/bin/ln -sf * /usr/lib/mysql/plugin/*），通过路径穿越将 /usr/bin/ln 软链接至 /bin/bash，再次执行 sudo ln 时实际以 root 权限启动 Bash，完成权限提升。该靶机重点考察了 SSRF 对内网的深度利用、Gopher 协议构造技巧、隐写数据提取以及 sudo 软链接配置不当导致的提权风险。
---

## 靶机说明

**关键词**：SSRF 链式利用、Gopher 协议打内网、Flask SSTI 反弹 Shell、零宽字符隐写、sudo ln 路径穿越提权

**摘要**：本文记录了 MazeSec XIYI 靶机的渗透过程。选手通过 80 端口的 SSRF 漏洞，先后利用 dict 协议探测内网端口、tftp 协议读取源码，发现 2332 端口的中继服务和 2333 端口存在 Jinja2 SSTI 的 Flask 应用；借助 Gopher 协议构造恶意 POST 请求穿透 SSRF 边界，在 2333 端口触发模板注入并反弹 Shell，获取 www-data 权限；在 Web 目录中发现包含大量零宽字符的秘密文件，通过十六进制分析与二进制解码提取出 lemon 用户的凭证，成功登录 SSH；最后利用 lemon 的 sudo 权限（NOPASSWD: /usr/bin/ln -sf * /usr/lib/mysql/plugin/*），通过路径穿越将 /usr/bin/ln 软链接至 /bin/bash，再次执行 sudo ln 时实际以 root 权限启动 Bash，完成权限提升。该靶机重点考察了 SSRF 对内网的深度利用、Gopher 协议构造技巧、隐写数据提取以及 sudo 软链接配置不当导致的提权风险。
...
![](/images/maze-xiyi/3.png)

ssrf访问内网2332端口，提示`get reply.py`

![](/images/maze-xiyi/4.png)
...
![](/images/maze-xiyi/8.png)

ssh 登录

![](/images/maze-xiyi/9.png)
