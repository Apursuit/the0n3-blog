...
description: >-
  MazeSec AllUser 靶机 Writeup：OpenSSH 私钥格式转换、ssh2john 爆破、iptables LOG 注入、PHP 文件包含 Getshell、特殊文件名绕过。选手通过 SSH 服务探测捕获到十六进制的 OpenSSH 私钥数据，经 Python 脚本还原并封装为标准 PEM 格式；利用 ssh2john 提取哈希后通过 John 爆破得到私钥密码 0123456，结合公钥注释推断出用户名 sandu 并成功登录；在 sandu 用户环境下发现其拥有 sudo 权限执行 iptables，利用 LOG 目标的 --log-prefix 参数向内核日志 /var/log/kern.log 注入 PHP WebShell 代码；通过 SOCAT 将本地 80 端口转发至外网，利用 Web 服务的 LFI 漏洞包含受控的内核日志文件，触发代码执行并反弹 Shell 获取 www-data 权限；最终在 Web 根目录发现以 --help root password 命名的特殊文件，利用路径前缀 ./ 绕过命令行参数解析，读取 root 明文密码完成权限提升。该靶机重点考察了 SSH 密钥格式分析、日志注入导致的代码执行以及 Linux 特殊文件名的访问技巧。
---

## 靶机说明

**关键词**：OpenSSH 私钥格式转换、ssh2john 爆破、iptables LOG 注入、PHP 文件包含 Getshell、特殊文件名绕过

**摘要**：本文记录了 MazeSec AllUser 靶机的渗透过程。选手通过 SSH 服务探测捕获到十六进制的 OpenSSH 私钥数据，经 Python 脚本还原并封装为标准 PEM 格式；利用 ssh2john 提取哈希后通过 John 爆破得到私钥密码 0123456，结合公钥注释推断出用户名 sandu 并成功登录；在 sandu 用户环境下发现其拥有 sudo 权限执行 iptables，利用 LOG 目标的 --log-prefix 参数向内核日志 /var/log/kern.log 注入 PHP WebShell 代码；通过 SOCAT 将本地 80 端口转发至外网，利用 Web 服务的 LFI 漏洞包含受控的内核日志文件，触发代码执行并反弹 Shell 获取 www-data 权限；最终在 Web 根目录发现以 --help root password 命名的特殊文件，利用路径前缀 ./ 绕过命令行参数解析，读取 root 明文密码完成权限提升。该靶机重点考察了 SSH 密钥格式分析、日志注入导致的代码执行以及 Linux 特殊文件名的访问技巧。
...
![](/images/maze-alluser/10.png)

一个黑客视觉的字符流终端效果

![](/images/maze-alluser/11.png)
...
当命令参数以路径形式出现时，不会被解析为选项，因为./明确指定了这是一个文件路径，而不是命令行选项
