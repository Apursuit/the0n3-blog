...
sidebar: true
description: >-
  MazeSec Firebird 靶机 Writeup：AD CS ESC7 利用、LDAP 属性信息泄露、NTLM 哈希传递、PKINIT 域认证。选手从给定的普通域用户 ll104567 凭证出发，通过 LDAP 协议精细化枚举发现 111 用户的 info 字段泄露数据库服务账号密码，利用该凭证访问受保护的 DB_Vault 共享目录，获取 sublarge 用户的 NTLM 哈希；通过哈希传递（PTH）结合 WinRM 服务获取 sublarge 的交互式权限，枚举 AD CS 配置后发现该用户对证书颁发机构拥有 ESC7 类高危管理权限，可通过「添加 CA Officer → 启用 SubCA 模板 → 提交域管身份证书请求 → 强制签发被拒请求」的利用链，绕过模板注册限制签发域管理员有效证书；最终借助 PKINIT 认证获取域管 NTLM 哈希，完成域控接管。该靶机重点考察域内 LDAP 属性信息泄露排查、AD CS 权限配置缺陷（ESC7）利用、PKI 信任链滥用等域渗透核心技能，还原了企业 AD 环境中非漏洞类权限提升的典型攻击路径。
---
