...
description: >-
  MazeSec vm1 靶机 Writeup：/proc 进程信息泄露、Python Flask 沙箱逃逸、sudo tree --fromfile 任意文件读取、tree 输出重定向写文件、Cron 定时任务提权。选手通过端口扫描发现 9000 端口存在文件读取功能，利用该漏洞读取 /etc/passwd 确认 Docker 容器环境，并通过读取 /proc 目录下各进程的 cmdline 文件定位到容器内运行的 Python Agent 服务；分析源码发现其存在 evalcode 接口，可直接执行任意 Python 代码，借此反弹 Shell 获取 python、php 用户权限并读取对应 flag；利用 busybox nc 构造 HTTP 请求调用 Node.js 服务的 eval 接口，成功读取 node 用户的 flag，拼接得到 SSH 登录密码；通过 Hydra 爆破确定 SSH 用户名为 admin，登录后发现 admin 拥有 sudo 权限执行 tree 命令；最终利用 tree 的 --fromfile 参数既可以读取 /root/root.txt，又可以通过 -o 参数将恶意内容（Crontab 反弹 Shell 或 SSH 公钥）以 root 权限写入系统敏感路径，完成权限提升。该靶机重点考察了 Linux /proc 文件系统信息收集、多语言代码执行漏洞的链式利用以及 sudo 中非常规二进制程序（tree）的权限滥用技巧。
---
...
**关键词**：/proc 进程信息泄露、Python Flask 沙箱逃逸、sudo tree --fromfile 任意文件读取、tree 输出重定向写文件、Cron 定时任务提权

**摘要**：本文记录了 MazeSec vm1 靶机的渗透过程。选手通过端口扫描发现 9000 端口存在文件读取功能，利用该漏洞读取 /etc/passwd 确认 Docker 容器环境，并通过读取 /proc 目录下各进程的 cmdline 文件定位到容器内运行的 Python Agent 服务；分析源码发现其存在 evalcode 接口，可直接执行任意 Python 代码，借此反弹 Shell 获取 python、php 用户权限并读取对应 flag；利用 busybox nc 构造 HTTP 请求调用 Node.js 服务的 eval 接口，成功读取 node 用户的 flag，拼接得到 SSH 登录密码；通过 Hydra 爆破确定 SSH 用户名为 admin，登录后发现 admin 拥有 sudo 权限执行 tree 命令；最终利用 tree 的 --fromfile 参数既可以读取 /root/root.txt，又可以通过 -o 参数将恶意内容（Crontab 反弹 Shell 或 SSH 公钥）以 root 权限写入系统敏感路径，完成权限提升。该靶机重点考察了 Linux /proc 文件系统信息收集、多语言代码执行漏洞的链式利用以及 sudo 中非常规二进制程序（tree）的权限滥用技巧。
...
![](/images/maze-vm1/1.png)


注释里发现关键信息

![](/images/maze-vm1/2.png)


可以读取文件

![](/images/maze-vm1/3.png)
...
![](/images/maze-vm1/10.png)

![](/images/maze-vm1/11.png)

靶机没有bash

![](/images/maze-vm1/12.png)

根目录存在 flag_node、flag_php、flag_py 三个 flag 文件，只能对应的三个用户读取

![](/images/maze-vm1/14.png)


可以读取到 php 源码，nodejs 源码

![](/images/maze-vm1/13.png)
...
![](/images/maze-vm1/18.png)

## sudo 权限枚举

admin 用户有 sudo tree 权限

![](/images/maze-vm1/19.png)

## 方法一：--fromfile 参数读取文件

tree 命令帮助参数里，--fromfile 选项，可以从文件里读取内容，以树状形式展示

![](/images/maze-vm1/20.png)

测试

![](/images/maze-vm1/21.png)

![](/images/maze-vm1/22.png)

tree 的 --fromfile 选项可以读取任意文件内容，以树状形式展示，sudo tree 直接读取 root.txt

![](/images/maze-vm1/23.png)
