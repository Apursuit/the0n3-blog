...
sidebar: true
description: >-
  MazeSec badbox 靶机设计文档：Alpine Linux、BusyBox 降权机制、SUID Bash 利用、/var/tmp 持久化、WordPress 环境搭建。重点展示了 Alpine Linux 下 SUID 提权的特殊性：即使获取了 ruid=1000, euid=0 的 Bash 进程，由于 BusyBox 的 check_suid 机制会在运行时检测 ruid 并主动放弃特权，导致 ls、cat 等工具降权执行，造成“伪 root Shell”现象。靶机基于 Alpine 构建了 Apache2 + PHP84 + MariaDB 的 WordPress 环境，通过 root 定时任务在避开 nosuid 限制的 /var/tmp 目录下部署 SUID Bash，为选手提供了稳定的提权锚点。该设计强制选手理解 SUID 进程凭证与 BusyBox 运行时行为的差异，并通过使用独立二进制工具或直接利用 Shell 内置命令来完成最终的权限控制，旨在填补传统提权教学中对 Alpine/BusyBox 体系认知的空白。
---
