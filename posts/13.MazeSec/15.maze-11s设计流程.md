---
title: 矛计划 MazeSec 11s - Windows靶机设计流程
...
sidebar: true
description: >-
  MazeSec 11s 靶机设计文档：SeImpersonatePrivilege 滥用、Potato 提权链、WinRM 横向移动、IIS 匿名认证降权、Apache 服务身份隔离、Windows 权限收敛。构建了一条从 Web 信息泄露到 SYSTEM 权限获取的经典 Windows 提权链。靶机通过精细化权限配置制造“环境陷阱”：Apache 以低权用户 kaada 运行并遗留 .git 泄露，诱导选手通过 SMB 爆破获取 WinRM 登录权限；IIS 则刻意配置为仅监听 127.0.0.1，迫使选手利用端口转发访问内网文件上传服务，并通过修改匿名认证身份为应用池账户解决 401.3 权限错误，最终获取具备 SeImpersonatePrivilege 的 WebShell。设计核心在于利用 Windows 服务账户默认持有的模拟特权，结合 Potato 系列漏洞（如 SweetPotato）实现权限提升，同时通过对 Apache 目录的写权限收紧与 IIS 上传目录的精确授权，避免非预期解法。该靶机重点考察选手对 Windows 服务身份隔离、令牌模拟机制及内网端口转发等实战能力的综合理解。
---
...
**关键词**：SeImpersonatePrivilege 滥用、Potato 提权链、WinRM 横向移动、IIS 匿名认证降权、Apache 服务身份隔离、Windows 权限收敛

**摘要**：本文为 MazeSec 11s Windows 靶机的设计文档，构建了一条从 Web 信息泄露到 SYSTEM 权限获取的经典 Windows 提权链。靶机通过精细化权限配置制造“环境陷阱”：Apache 以低权用户 kaada 运行并遗留 .git 泄露，诱导选手通过 SMB 爆破获取 WinRM 登录权限；IIS 则刻意配置为仅监听 127.0.0.1，迫使选手利用端口转发访问内网文件上传服务，并通过修改匿名认证身份为应用池账户解决 401.3 权限错误，最终获取具备 SeImpersonatePrivilege 的 WebShell。设计核心在于利用 Windows 服务账户默认持有的模拟特权，结合 Potato 系列漏洞（如 SweetPotato）实现权限提升，同时通过对 Apache 目录的写权限收紧与 IIS 上传目录的精确授权，避免非预期解法。该靶机重点考察选手对 Windows 服务身份隔离、令牌模拟机制及内网端口转发等实战能力的综合理解。
