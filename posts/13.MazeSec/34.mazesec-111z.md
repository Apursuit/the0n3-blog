...
description: >-
  MazeSec 111z 靶机 Writeup：PHP disable_functions 绕过、LD_PRELOAD 劫持、sudo neofetch 配置滥用、tar 通配符注入、SSH 凭证泄露。选手通过审计前端 JS 发现上传限制，利用 PHP 标签闭合省略分号及函数名大小写混淆绕过 WAF，成功上传 WebShell；面对严格的 disable_functions 限制，借助蚁剑插件利用 putenv 与 error_log 函数配合 LD_PRELOAD 劫持实现命令执行，或通过文件读取功能直接获取 /opt/llpass.txt 中的 SSH 凭证；使用 ll 用户登录后，发现其拥有 sudo 权限执行 neofetch，通过构造恶意配置文件触发 exec bash 切换至 mj 用户；在 mj 用户环境下审计备份脚本，利用 tar 命令的通配符注入漏洞（--checkpoint-action=exec），在压缩目录中植入恶意文件名触发 root 权限执行的 Shell 脚本，最终生成 SUID 位的 bash 完成权限提升。该靶机重点考察了 PHP 安全防护机制的绕过、sudo 中非标准二进制程序的滥用以及归档工具通配符注入的经典提权手法。
---


## 靶机说明

**关键词**：PHP disable_functions 绕过、LD_PRELOAD 劫持、sudo neofetch 配置滥用、tar 通配符注入、SSH 凭证泄露

**摘要**：本文记录了 MazeSec 111z 靶机的渗透过程。选手通过审计前端 JS 发现上传限制，利用 PHP 标签闭合省略分号及函数名大小写混淆绕过 WAF，成功上传 WebShell；面对严格的 disable_functions 限制，借助蚁剑插件利用 putenv 与 error_log 函数配合 LD_PRELOAD 劫持实现命令执行，或通过文件读取功能直接获取 /opt/llpass.txt 中的 SSH 凭证；使用 ll 用户登录后，发现其拥有 sudo 权限执行 neofetch，通过构造恶意配置文件触发 exec bash 切换至 mj 用户；在 mj 用户环境下审计备份脚本，利用 tar 命令的通配符注入漏洞（--checkpoint-action=exec），在压缩目录中植入恶意文件名触发 root 权限执行的 Shell 脚本，最终生成 SUID 位的 bash 完成权限提升。该靶机重点考察了 PHP 安全防护机制的绕过、sudo 中非标准二进制程序的滥用以及归档工具通配符注入的经典提权手法。
...
![](/images/maze-111z/27.png)



![](/images/maze-111z/28.png)


扫描 `/opt` 目录，发现 `llpass.txt` 文件，拿到 ll 用户的 ssh 密码

![](/images/maze-111z/30.png)
...
![](/images/maze-111z/8.png)

几乎禁用了所有危险函数

![](/images/maze-111z/2.png)
...
![](/images/maze-111z/16.png)

![](/images/maze-111z/17.png)
