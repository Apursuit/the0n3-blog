...
sidebar: true
description: >-
  MazeSec HackMe 靶机 Writeup：PHP反序列化、ARP命令注入、图片隐写、Linux提权。通过访问 80 端口发现的 JPEG 图片，利用 strings 工具提取出尾部隐藏的 PHP 反序列化代码，构造利用链获取初始 WebShell；在系统内发现 root 权限运行的 ARP 监听服务，通过分析服务逻辑并利用 Scapy 构造携带 CMD: 前缀的 ARP 广播包，绕过空格与重定向符过滤，实现无空格命令执行；最终通过该 ARP 后门直接以 root 权限读写系统文件，完成权限提升。该靶机重点考察了隐写分析、PHP 反序列化利用及非常规网络协议后门的挖掘能力。
---
...
**关键词**：PHP反序列化、ARP命令注入、图片隐写、Linux提权

**摘要**：本文记录了 MazeSec HackMe 靶机的渗透过程。通过访问 80 端口发现的 JPEG 图片，利用 strings 工具提取出尾部隐藏的 PHP 反序列化代码，构造利用链获取初始 WebShell；在系统内发现 root 权限运行的 ARP 监听服务，通过分析服务逻辑并利用 Scapy 构造携带 CMD: 前缀的 ARP 广播包，绕过空格与重定向符过滤，实现无空格命令执行；最终通过该 ARP 后门直接以 root 权限读写系统文件，完成权限提升。该靶机重点考察了隐写分析、PHP 反序列化利用及非常规网络协议后门的挖掘能力。
