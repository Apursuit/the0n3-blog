...
permalink: /posts/maze-sandplanet/
tags: 
- MazeSec
categories: 
- kubernetes
...
sidebar: true
description: >-
  MazeSec SandPlanet 靶机 Writeup：Kubernetes容器逃逸、core_pattern劫持、OverlayFS路径泄露、WPA握手包破解、内网未授权横向。首先从Web入口突破：通过主题关联弱口令admin/sandplanet登录后台，利用文件上传漏洞获取www-data权限Webshell；随后开展内网渗透：发现环境变量指向的未授权跳板服务，直接获取跳板机权限，破解捕获的WPA握手包得到可复用密码，扫描内网后SSH登录目标业务容器；最终完成容器逃逸：利用容器内/proc/sys/kernel/core_pattern可写、OverlayFS挂载路径可映射至宿主机的特性，构造反弹Shell脚本并劫持core_pattern，通过触发程序崩溃以宿主机root权限执行恶意代码，成功获取宿主机控制权。该靶机综合考察了Web渗透、无线密码破解、内网横向及云原生容器逃逸等实战技能，尤其侧重OverlayFS存储特性与Linux内核机制的利用思路。
---
...
**关键词**：Kubernetes容器逃逸、core_pattern劫持、OverlayFS路径泄露、WPA握手包破解、内网未授权横向

**摘要**：本文记录了MazeSec SandPlanet云原生靶机的渗透全过程。首先从Web入口突破：通过主题关联弱口令admin/sandplanet登录后台，利用文件上传漏洞获取www-data权限Webshell；随后开展内网渗透：发现环境变量指向的未授权跳板服务，直接获取跳板机权限，破解捕获的WPA握手包得到可复用密码，扫描内网后SSH登录目标业务容器；最终完成容器逃逸：利用容器内/proc/sys/kernel/core_pattern可写、OverlayFS挂载路径可映射至宿主机的特性，构造反弹Shell脚本并劫持core_pattern，通过触发程序崩溃以宿主机root权限执行恶意代码，成功获取宿主机控制权。该靶机综合考察了Web渗透、无线密码破解、内网横向及云原生容器逃逸等实战技能，尤其侧重OverlayFS存储特性与Linux内核机制的利用思路。
