---
title: MazeSec Meltdown
date: 2026-01-04 17:40:34
permalink: /pages/maze-meltdown/
categories:
  - MazeSec
tags:
  - 换行符注入
...
description: >-
  MazeSec Meltdown 靶机 Writeup：SQL 注入后台 Getshell、存储型 PHP 代码执行、sudo repeater.sh 过滤绕过、grep 按行匹配缺陷、换行符注入。选手通过 SQL 注入漏洞获取后台用户 rin 的明文凭证，登录后台后在物品描述字段写入 PHP 代码，利用后端直接解析存储内容的特性触发代码执行，获取 www-data 权限；在 rin 用户环境下发现其拥有 sudo 权限执行 /opt/repeater.sh 脚本，审计脚本发现过滤逻辑存在多处缺陷：grep 按行匹配的特性使得仅单行无空格的输入可绕过格式校验，eval 语句未严格处理用户输入导致引号注入，结合换行符注入可绕过空格限制，最终实现以 root 权限执行任意命令，完成权限提升。该靶机重点考察了 SQL 注入的信息利用、存储型代码执行的风险以及 sudo 自定义脚本中常见的过滤逻辑缺陷挖掘与绕过技巧。
---
...
**关键词**：SQL 注入后台 Getshell、存储型 PHP 代码执行、sudo repeater.sh 过滤绕过、grep 按行匹配缺陷、换行符注入

**摘要**：本文记录了 MazeSec Meltdown 靶机的渗透过程。选手通过 SQL 注入漏洞获取后台用户 rin 的明文凭证，登录后台后在物品描述字段写入 PHP 代码，利用后端直接解析存储内容的特性触发代码执行，获取 www-data 权限；在 rin 用户环境下发现其拥有 sudo 权限执行 /opt/repeater.sh 脚本，审计脚本发现过滤逻辑存在多处缺陷：grep 按行匹配的特性使得仅单行无空格的输入可绕过格式校验，eval 语句未严格处理用户输入导致引号注入，结合换行符注入可绕过空格限制，最终实现以 root 权限执行任意命令，完成权限提升。该靶机重点考察了 SQL 注入的信息利用、存储型代码执行的风险以及 sudo 自定义脚本中常见的过滤逻辑缺陷挖掘与绕过技巧。
...
![](/images/maze-meltdown/4.png)

进入物品管理，物品介绍内容里写入 php 代码

![](/images/maze-meltdown/5.png)

返回查看物品，可以发现代码执行

![](/images/maze-meltdown/6.png)
...
<video width="600" controls>
  <source src="/images/maze-meltdown/demo.mp4" type="video/mp4">
  演示
</video>
...
![](/images/maze-meltdown/10.png)

![](/images/maze-meltdown/11.png)

换成 sudo 执行即可

![](/images/maze-meltdown/12.png)


![](/images/maze-meltdown/13.png)

甚至实现空格自由，执行个反弹 Shell 命令

![](/images/maze-meltdown/14.png)

![](/images/maze-meltdown/15.png)
