...
sidebar: true
description: >-
  MazeSec Hyperjail 靶机 Writeup：libvirt未授权访问、存储池路径篡改、QEMU磁盘挂载、NBD离线提权、KVM虚拟机逃逸。通过识别 16509 端口运行的 libvirtd 服务存在未授权访问漏洞，利用 virsh 客户端直接接管宿主机虚拟化管理层；借助 libvirt 存储池（Storage Pool）的路径篡改能力，先后实现 /root/.ssh/authorized_keys 写入与 /etc/crontabs/ 定时任务植入，成功获取宿主机 root 权限；随后发现 user flag 位于 KVM 虚拟机内部，通过卸载虚拟机磁盘镜像、挂载至宿主机、离线注入 SSH 公钥的方式绕过虚拟机防护，最终通过 2222 端口 SSH 登录虚拟机获取用户凭证。该靶机重点考察了虚拟化基础设施的权限管控缺陷、libvirt API 的滥用风险以及 NBD/QEMU 离线磁盘操作的实战技巧。
---
