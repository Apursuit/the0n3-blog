...
description: >-
  MazeSec Scanner 靶机 Writeup：换行符绕过 WAF、sudo ln 软链接劫持、sudo sd 正则替换、Nikto 插件劫持提权、SSH 公钥注入。选手通过 8080 端口 Apache 服务的 IP 检测功能，利用 %0a 换行符绕过 WAF 实现命令执行，获取 www-data 权限；在受限的可写目录环境中，利用 sudo 配置缺陷（(welcome) NOPASSWD: /usr/bin/ln, /usr/bin/sd），通过 ln 创建软链接将 .ssh 目录指向家目录，并结合 sd 的正则替换功能将 .bashrc 内容改写为攻击机公钥，最终通过硬链接将公钥映射为 authorized_keys，成功登录 welcome 用户；提权阶段发现 welcome 拥有 NOPASSWD 权限执行 /usr/bin/nikto，通过构造恶意插件并指定自定义配置文件劫持插件加载路径，触发 Nikto 以 root 权限执行 exec("/bin/sh -p")，完成权限提升。该靶机重点考察了非常规命令注入、Linux 链接文件特性以及第三方安全工具的配置滥用技巧。
---

## 靶机说明

**关键词**：换行符绕过 WAF、sudo ln 软链接劫持、sudo sd 正则替换、Nikto 插件劫持提权、SSH 公钥注入

**摘要**：本文记录了 MazeSec Scanner 靶机的渗透过程。选手通过 8080 端口 Apache 服务的 IP 检测功能，利用 %0a 换行符绕过 WAF 实现命令执行，获取 www-data 权限；在受限的可写目录环境中，利用 sudo 配置缺陷（(welcome) NOPASSWD: /usr/bin/ln, /usr/bin/sd），通过 ln 创建软链接将 .ssh 目录指向家目录，并结合 sd 的正则替换功能将 .bashrc 内容改写为攻击机公钥，最终通过硬链接将公钥映射为 authorized_keys，成功登录 welcome 用户；提权阶段发现 welcome 拥有 NOPASSWD 权限执行 /usr/bin/nikto，通过构造恶意插件并指定自定义配置文件劫持插件加载路径，触发 Nikto 以 root 权限执行 exec("/bin/sh -p")，完成权限提升。该靶机重点考察了非常规命令注入、Linux 链接文件特性以及第三方安全工具的配置滥用技巧。
...
![](/images/Scanner/2.png)


扫描目录

![](/images/Scanner/1.png)
...
![](/images/Scanner/10.png)

使用ln命令，创建软连接、硬链接

软链接可以指向文件和目录，硬链接只能指向文件

![](/images/Scanner/11.png)
...
![](/images/Scanner/14.png)

总结下：

其中，/home/user/key 实际存放公钥。

```bash
ln -sfn /home/user/ /home/user/.ssh
ln -f /home/user/key /home/user/.ssh/authorized_keys
```
