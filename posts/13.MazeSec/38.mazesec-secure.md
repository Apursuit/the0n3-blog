...
description: >-
  MazeSec Secure 靶机 Writeup：DVWA 命令执行、LinPEAS 凭证扫描、SSH AuthorizedKeysFile 路径劫持、PKCS#11 动态库劫持、sudo ssh-keygen 提权。选手通过目录扫描发现 DVWA 靶场，利用 Low 级别命令执行模块反弹 Shell 获取 www-data 权限；运行 LinPEAS 审计脚本，从 su 爆破尝试中提取出用户 lzh 的明文密码 hzl；通过查找近期变更文件发现 SSH 服务配置被修改为 AuthorizedKeysFile /tmp/authorized_keys2 且关闭了 StrictModes 校验；利用 lzh 用户在 /tmp 目录下写入攻击机公钥，成功横向移动至 one3 用户；最终发现 one3 拥有 sudo 权限执行 ssh-keygen，通过编写包含构造函数属性的恶意 PKCS#11 动态库，利用 ssh-keygen 的 -D 参数加载外部库的特性，在 root 上下文中执行代码并生成 SUID 位的 bash，完成权限提升。该靶机重点考察了弱靶场利用、自动化提权脚本的信息收集能力、SSH 配置不当导致的认证绕过以及加密工具库的代码执行漏洞利用。
---


## 靶机说明

**关键词**：DVWA 命令执行、LinPEAS 凭证扫描、SSH AuthorizedKeysFile 路径劫持、PKCS#11 动态库劫持、sudo ssh-keygen 提权

**摘要**：本文记录了 MazeSec Secure 靶机的渗透过程。选手通过目录扫描发现 DVWA 靶场，利用 Low 级别命令执行模块反弹 Shell 获取 www-data 权限；运行 LinPEAS 审计脚本，从 su 爆破尝试中提取出用户 lzh 的明文密码 hzl；通过查找近期变更文件发现 SSH 服务配置被修改为 AuthorizedKeysFile /tmp/authorized_keys2 且关闭了 StrictModes 校验；利用 lzh 用户在 /tmp 目录下写入攻击机公钥，成功横向移动至 one3 用户；最终发现 one3 拥有 sudo 权限执行 ssh-keygen，通过编写包含构造函数属性的恶意 PKCS#11 动态库，利用 ssh-keygen 的 -D 参数加载外部库的特性，在 root 上下文中执行代码并生成 SUID 位的 bash，完成权限提升。该靶机重点考察了弱靶场利用、自动化提权脚本的信息收集能力、SSH 配置不当导致的认证绕过以及加密工具库的代码执行漏洞利用。
...
![](/images/maze-secure/2.png)

![](/images/maze-secure/3.png)

进入命令执行模块，反弹个shell 过来

![](/images/maze-secure/4.png)

稳定优化 shell

![](/images/maze-secure/5.png)
