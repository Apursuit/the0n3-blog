...
description: >-
  MazeSec motto 靶机 Writeup：二阶 SQL 注入、sqlmap 自动化利用、密码复用、bash 比较注入、SUID bash 提权。选手通过端口扫描发现 9090 端口运行着定制 Web 服务，在修改用户昵称处插入恶意载荷后发现座右铭展示页面出现了数据泄露，确认为二阶 SQL 注入漏洞；利用 sqlmap 的 --second-url 参数自动化攻击，从数据库中导出用户凭证，使用 RedBean/cannotforgetyou 成功登录 SSH；在 redbean 用户目录下发现 .backup 备份文件夹，审计其中的 C 源码与 Shell 脚本发现 run_newsh 设置了 SUID 位且内部调用了存在缺陷的 new.sh；利用 bash 脚本中未加引号的变量比较漏洞（$1 = "flag" 与 [ $1 = "flag" ] 的解析差异），传入含空格的参数绕过安全限制，触发 chmod +s /bin/bash 逻辑；最终执行带权限的 bash -p 获取 root 权限。该靶机重点考察了二阶注入的联动利用、源码审计以及 Shell 脚本因引号缺失导致的权限提升风险。
---

## 靶机说明

**关键词**：二阶 SQL 注入、sqlmap 自动化利用、密码复用、bash 比较注入、SUID bash 提权

**摘要**：本文记录了 MazeSec motto 靶机的渗透过程。选手通过端口扫描发现 9090 端口运行着定制 Web 服务，在修改用户昵称处插入恶意载荷后发现座右铭展示页面出现了数据泄露，确认为二阶 SQL 注入漏洞；利用 sqlmap 的 --second-url 参数自动化攻击，从数据库中导出用户凭证，使用 RedBean/cannotforgetyou 成功登录 SSH；在 redbean 用户目录下发现 .backup 备份文件夹，审计其中的 C 源码与 Shell 脚本发现 run_newsh 设置了 SUID 位且内部调用了存在缺陷的 new.sh；利用 bash 脚本中未加引号的变量比较漏洞（$1 = "flag" 与 [ $1 = "flag" ] 的解析差异），传入含空格的参数绕过安全限制，触发 chmod +s /bin/bash 逻辑；最终执行带权限的 bash -p 获取 root 权限。该靶机重点考察了二阶注入的联动利用、源码审计以及 Shell 脚本因引号缺失导致的权限提升风险。
...
![](/images/maze-motto/1.png)

## 3. 二阶 SQL 注入发现

在 `/myinfo` 修改昵称为 `'or 1='1` 后，到 `/mymottos` 页面发现出现所有用户的 motto，推测存在二阶 SQL 注入（源头在个人信息保存，生效点在座右铭查询）。

![](/images/maze-motto/2.png)
![](/images/maze-motto/3.png)
![](/images/maze-motto/4.png)
