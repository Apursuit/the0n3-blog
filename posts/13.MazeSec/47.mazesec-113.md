...
description: >-
  MazeSec 113 靶机 Writeup：SNMP 信息泄露、sudo 脚本变量注入、Bash 数组索引命令替换、PATH 环境变量劫持、IFS 分隔符篡改。选手通过 UDP 161 端口的 SNMP 服务，利用默认的 public 社区名通过 snmpbulkwalk 枚举系统进程信息，成功泄露 welcome 用户的明文密码；SSH 登录后审计 sudo 权限，发现 welcome 可无密码执行 /opt/113.sh 脚本。该脚本存在多处逻辑缺陷：其一，利用 Bash 变量默认以数组形式存储的特性，通过声明 exec_[0] 绕过变量名黑名单限制；其二，利用 Bash 数组索引支持命令替换的语法特性，在索引位置执行任意命令；其三，通过劫持 PATH 环境变量，使沙箱内执行的 mazesec 脚本调用恶意版本的 md5sum 或 awk 命令，生成 SUID bash；其四，通过修改 IFS 内部字段分隔符，将沙箱路径中的点号 . 作为分隔符，诱导脚本将路径前半段解析为可执行文件进行执行。最终多种方案均成功获取 root 权限。该靶机重点考察了对 SNMP 服务的信息收集能力、Linux Shell 脚本的高级变量解析机制以及环境变量滥用导致的提权技巧。
---
...
**关键词**：SNMP 信息泄露、sudo 脚本变量注入、Bash 数组索引命令替换、PATH 环境变量劫持、IFS 分隔符篡改

**摘要**：本文记录了 MazeSec 113 靶机的渗透过程。选手通过 UDP 161 端口的 SNMP 服务，利用默认的 public 社区名通过 snmpbulkwalk 枚举系统进程信息，成功泄露 welcome 用户的明文密码；SSH 登录后审计 sudo 权限，发现 welcome 可无密码执行 /opt/113.sh 脚本。该脚本存在多处逻辑缺陷：其一，利用 Bash 变量默认以数组形式存储的特性，通过声明 exec_[0] 绕过变量名黑名单限制；其二，利用 Bash 数组索引支持命令替换的语法特性，在索引位置执行任意命令；其三，通过劫持 PATH 环境变量，使沙箱内执行的 mazesec 脚本调用恶意版本的 md5sum 或 awk 命令，生成 SUID bash；其四，通过修改 IFS 内部字段分隔符，将沙箱路径中的点号 . 作为分隔符，诱导脚本将路径前半段解析为可执行文件进行执行。最终多种方案均成功获取 root 权限。该靶机重点考察了对 SNMP 服务的信息收集能力、Linux Shell 脚本的高级变量解析机制以及环境变量滥用导致的提权技巧。
...
![](/images/maze-113/3.png)

尝试 ssh 登录 welcome 用户

![](/images/maze-113/4.png)
