...
sidebar: true
description: >-
  MazeSec ED 靶机 Writeup：tcpdump流量分析、软链接劫持、sudo滥用、Linux提权。通过页面提示“Listen carefully”联想到流量分析，利用 tcpdump 捕获局域网广播流量，获取 backupd 用户的 SSH 私钥并成功登录；在拥有 sudo reboot 权限的前提下，结合 root 定时执行的广播脚本存在的缺陷，利用软链接分别实现任意文件读取与 SSH 配置篡改；最终通过修改脚本植入 SUID 后门，成功获取 root 权限。该靶机重点考察了对系统服务逻辑的逆向分析能力与 Linux 基础提权技巧的综合运用。
---
...
**关键词**：tcpdump流量分析、软链接劫持、sudo滥用、Linux提权

**摘要**：本文记录了 MazeSec ED 靶机的渗透过程。通过页面提示“Listen carefully”联想到流量分析，利用 tcpdump 捕获局域网广播流量，获取 backupd 用户的 SSH 私钥并成功登录；在拥有 sudo reboot 权限的前提下，结合 root 定时执行的广播脚本存在的缺陷，利用软链接分别实现任意文件读取与 SSH 配置篡改；最终通过修改脚本植入 SUID 后门，成功获取 root 权限。该靶机重点考察了对系统服务逻辑的逆向分析能力与 Linux 基础提权技巧的综合运用。
