...
description: >-
  MazeSec Spiteful 靶机 Writeup：换行符注入绕过 WAF、MySQL LOAD DATA 读 Shadow、sudo rkhunter 日志注入、Cron 定时任务写文件、Alpine 容器提权。选手通过 Web 暴力破解获取 ll104567 用户凭证，在受限的命令执行界面利用允许的命令拼接换行符 %0a 绕过正则拦截，通过 BusyBox 下载 Webshell 获取 www-data 权限；审计源码发现 todd 用户的 SSH 凭证，登录后利用 sudo 权限以 rkhunter 身份运行 Web 服务，再次通过弱口令进入管理后台并提取 MySQL root 哈希；利用 MySQL 用户属于 shadow 组的权限配置缺陷，通过 LOAD DATA INFILE 读取 /etc/shadow 文件，使用 John 爆破出 rkhunter 用户密码；最终利用 sudo rkhunter 的 --tmpdir 参数存在换行注入的特性，将恶意 Cron 表达式写入日志文件并指定输出至 /etc/crontabs/root，通过 BusyBox crond 每分钟执行一次反向 Shell，成功获取 root 权限。该靶机重点考察了输入过滤绕过、Linux 权限隔离缺陷以及利用程序日志功能实现文件写入的高级提权技巧。
---

## 靶机说明

**关键词**：换行符注入绕过 WAF、MySQL LOAD DATA 读 Shadow、sudo rkhunter 日志注入、Cron 定时任务写文件、Alpine 容器提权

**摘要**：本文记录了 MazeSec Spiteful 靶机的渗透过程。选手通过 Web 暴力破解获取 ll104567 用户凭证，在受限的命令执行界面利用允许的命令拼接换行符 %0a 绕过正则拦截，通过 BusyBox 下载 Webshell 获取 www-data 权限；审计源码发现 todd 用户的 SSH 凭证，登录后利用 sudo 权限以 rkhunter 身份运行 Web 服务，再次通过弱口令进入管理后台并提取 MySQL root 哈希；利用 MySQL 用户属于 shadow 组的权限配置缺陷，通过 LOAD DATA INFILE 读取 /etc/shadow 文件，使用 John 爆破出 rkhunter 用户密码；最终利用 sudo rkhunter 的 --tmpdir 参数存在换行注入的特性，将恶意 Cron 表达式写入日志文件并指定输出至 /etc/crontabs/root，通过 BusyBox crond 每分钟执行一次反向 Shell，成功获取 root 权限。该靶机重点考察了输入过滤绕过、Linux 权限隔离缺陷以及利用程序日志功能实现文件写入的高级提权技巧。
...
![](/images/maze-spiteful/15.png)

![](/images/maze-spiteful/16.png)
    

一个登录框，使用 burp intruder 爆破，爆破出 admin:raprap

![](/images/maze-spiteful/17.png)

![](/images/maze-spiteful/18.png)

在进程里，有个扎眼的 mysql 服务，一不小心使用 admin：raprap 登录成功

![](/images/maze-spiteful/19.png)

![](/images/maze-spiteful/20.png)
...
![](/images/maze-spiteful/29.png)


等待一分钟，kali 监听端口，拿到 root shell

![](/images/maze-spiteful/30.png)

> man,goodnight!

![](/images/maze-spiteful/31.png)