...
description: >-
  MazeSec SudoHome 靶机 Writeup：sudo 链式提权、--files0-from 信息泄露、ProxyCommand 逃逸、wfuzz Payload 回显、md5sum 字典爆破、软链接劫持。选手通过 SSH 登录提示发现 user1 用户，利用其 sudo 权限逐层向下跳转：user1 通过 sudo du --files0-from 参数泄露 user2 密码；user2 利用 sudo file -f 报错回显获取 user3 凭证；user3 借助 sudo mc -U 直接进入 user4 的 subshell；user4 通过 sudo ssh ProxyCommand 逃逸获得 user5 权限；user5 使用 sudo rev 双重反转读取 user6 密码；user6 利用 sudo cp 覆盖 .profile 窃取 user7 凭证；user7 通过 sudo mail 交互式 shell 切换至 user8；user8 借助 sudo wfuzz 将密码文件作为字典，在 Payload 列中泄露 user9 密码；user9 利用 sudo md5sum 配合 rockyou.txt 字典爆破出 user10 的明文密码 morrinsville；最终 user10 利用 sudo cat 配合家目录写权限，删除并重建指向 /root/.ssh/id_rsa 的软链接，读取 root 私钥完成权限提升。该靶机重点考察了 sudo 配置中各类二进制工具的非常规利用手法以及链式权限提升的完整逻辑。
---

## 靶机说明

**关键词**：sudo 链式提权、--files0-from 信息泄露、ProxyCommand 逃逸、wfuzz Payload 回显、md5sum 字典爆破、软链接劫持

**摘要**：本文记录了 MazeSec SudoHome 靶机的渗透过程。选手通过 SSH 登录提示发现 user1 用户，利用其 sudo 权限逐层向下跳转：user1 通过 sudo du --files0-from 参数泄露 user2 密码；user2 利用 sudo file -f 报错回显获取 user3 凭证；user3 借助 sudo mc -U 直接进入 user4 的 subshell；user4 通过 sudo ssh ProxyCommand 逃逸获得 user5 权限；user5 使用 sudo rev 双重反转读取 user6 密码；user6 利用 sudo cp 覆盖 .profile 窃取 user7 凭证；user7 通过 sudo mail 交互式 shell 切换至 user8；user8 借助 sudo wfuzz 将密码文件作为字典，在 Payload 列中泄露 user9 密码；user9 利用 sudo md5sum 配合 rockyou.txt 字典爆破出 user10 的明文密码 morrinsville；最终 user10 利用 sudo cat 配合家目录写权限，删除并重建指向 /root/.ssh/id_rsa 的软链接，读取 root 私钥完成权限提升。该靶机重点考察了 sudo 配置中各类二进制工具的非常规利用手法以及链式权限提升的完整逻辑。
...
![](/images/maze-sudohome/1.png)

![](/images/maze-sudohome/2.png)

> user1 : 0woA8Sr7I83R0ZwmnTcH

家目录有10个用户，每个用户目录下有一个password.txt文件

![](/images/maze-sudohome/3.png)

## user2 du 读取文件

user1用户可以无密码使用 user2 的 du 命令，--help看看帮助

![](/images/maze-sudohome/4.png)
...
![](/images/maze-sudohome/7.png)

mc 命令可以进入子shell或编辑文件

![](/images/maze-sudohome/8.png)
...
![](/images/maze-sudohome/9.png)

![](/images/maze-sudohome/10.png)
...
![](/images/maze-sudohome/16.png)

![](/images/maze-sudohome/17.png)
...
![](/images/maze-sudohome/26.png)

![](/images/maze-sudohome/27.png)
