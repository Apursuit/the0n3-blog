...
description: >-
  MazeSec 112 靶机 Writeup：XXE 任意文件读取、掩码密码爆破、sudo 脚本路径解析劫持、Zabbix 数据库初始化、静态 PHP 编译部署、nmap Lua 提权。选手通过 80 端口的 XML 解析接口发现 XXE 漏洞，利用 file:// 协议读取 /etc/passwd，定位到用户 tuf 并从其注释中提取掩码密码线索；通过 Python 脚本生成 3844 种两位字符组合爆破 SSH，成功获取 tuf 用户权限；在 sudo 权限审计中发现 tuf 可无密码执行 /opt/112.sh，该脚本存在 URL 格式校验缺陷，利用多斜杠等价特性与 shell 对含 / 参数的路径解析规则，通过覆盖脚本内容并构造 https://maze-sec.com/111 目录结构，诱导 sudo 执行当前目录下的恶意脚本，最终通过写入 SUID bash 完成提权；此外靶机预留了“毛坯房”利用链，通过补全缺失的 Zabbix 数据库初始化流程、使用 static-php-cli 解决 Web 前端依赖问题，部署 Zabbix 5.0.8 后台并利用脚本执行功能获取 zabbix 用户权限，最终通过 sudo nmap 加载恶意 Lua 脚本实现 root 权限提升。该靶机重点考察了 XXE 信息收集、sudo 脚本逻辑缺陷的深度利用、企业级监控系统的运维部署缺陷以及静态二进制工具在渗透测试中的实战价值。
---
...
**关键词**：XXE 任意文件读取、掩码密码爆破、sudo 脚本路径解析劫持、Zabbix 数据库初始化、静态 PHP 编译部署、nmap Lua 提权

**摘要**：本文记录了 MazeSec 112 靶机的渗透过程。选手通过 80 端口的 XML 解析接口发现 XXE 漏洞，利用 file:// 协议读取 /etc/passwd，定位到用户 tuf 并从其注释中提取掩码密码线索；通过 Python 脚本生成 3844 种两位字符组合爆破 SSH，成功获取 tuf 用户权限；在 sudo 权限审计中发现 tuf 可无密码执行 /opt/112.sh，该脚本存在 URL 格式校验缺陷，利用多斜杠等价特性与 shell 对含 / 参数的路径解析规则，通过覆盖脚本内容并构造 https://maze-sec.com/111 目录结构，诱导 sudo 执行当前目录下的恶意脚本，最终通过写入 SUID bash 完成提权；此外靶机预留了“毛坯房”利用链，通过补全缺失的 Zabbix 数据库初始化流程、使用 static-php-cli 解决 Web 前端依赖问题，部署 Zabbix 5.0.8 后台并利用脚本执行功能获取 zabbix 用户权限，最终通过 sudo nmap 加载恶意 Lua 脚本实现 root 权限提升。该靶机重点考察了 XXE 信息收集、sudo 脚本逻辑缺陷的深度利用、企业级监控系统的运维部署缺陷以及静态二进制工具在渗透测试中的实战价值。
...
![](/images/maze-112/8.png)

## 2.1、环境补充与 Web 部署

检查靶机是否注册了 zabbix 服务，zabbix 服务启动失败

![](/images/maze-112/9.png)

收集靶机 zabbix 服务相关信息

![](/images/maze-112/10.png)
...
![](/images/maze-112/11.png)

查看 zabbix 配置文件里的 日志文件内容

![](/images/maze-112/12.png)
...
![](/images/maze-112/16.png)


再次启动，缺少 php-bcmath 模块

```bash
php -S 0.0.0.0:8000 -d post_max_size=16M -d max_execution_time=300 -d max_input_time=300 -d memory_limit=128M -d extension=bcmath -d extension=gd -d extension=mbstring -d extension=xmlreader -d extension=xmlwriter
```

![](/images/maze-112/17.png)
...
![](/images/maze-112/23.png)

![](/images/maze-112/22.png)
...
![](/images/maze-112/24.png)

![](/images/maze-112/25.png)

在主机监控处触发反弹shell脚本

![](/images/maze-112/26.png)

![](/images/maze-112/27.png)
