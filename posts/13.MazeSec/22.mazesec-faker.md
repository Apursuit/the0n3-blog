...
description: >-
  MazeSec faker 靶机 Writeup：MSSQL TRUSTWORTHY 滥用、EXECUTE AS 用户模拟、xp_cmdshell 执行、MSSQL Agent Job 无文件落地、SeImpersonatePrivilege 提权。选手通过初始凭证 db-user:whoami 连接 MSSQL 数据库，枚举发现 faker 数据库开启 TRUSTWORTHY 属性且当前用户具备 db_owner 权限；利用 EXECUTE AS USER = 'dbo' 模拟高权限用户，成功启用 xp_cmdshell 读取 C:\flag\user.txt；随后依据 flag 提示发现 MSSQL Agent 服务运行，通过创建 Agent Job 调用 PowerShell 无文件落地执行反向 Shell，获取 NT SERVICE\SQLSERVERAGENT 权限；最终利用该账户持有的 SeImpersonatePrivilege，通过 Meterpreter getsystem 的命名管道模拟技术窃取 SYSTEM 令牌，完成权限提升。该靶机重点考察了 MSSQL 数据库配置缺陷利用、Agent Job 命令执行以及 Windows 服务账户特权滥用等域渗透核心技能。
---
...
**关键词**：MSSQL TRUSTWORTHY 滥用、EXECUTE AS 用户模拟、xp_cmdshell 执行、MSSQL Agent Job 无文件落地、SeImpersonatePrivilege 提权

**摘要**：本文记录了 MazeSec faker Windows 靶机的渗透过程。选手通过初始凭证 db-user:whoami 连接 MSSQL 数据库，枚举发现 faker 数据库开启 TRUSTWORTHY 属性且当前用户具备 db_owner 权限；利用 EXECUTE AS USER = 'dbo' 模拟高权限用户，成功启用 xp_cmdshell 读取 C:\flag\user.txt；随后依据 flag 提示发现 MSSQL Agent 服务运行，通过创建 Agent Job 调用 PowerShell 无文件落地执行反向 Shell，获取 NT SERVICE\SQLSERVERAGENT 权限；最终利用该账户持有的 SeImpersonatePrivilege，通过 Meterpreter getsystem 的命名管道模拟技术窃取 SYSTEM 令牌，完成权限提升。该靶机重点考察了 MSSQL 数据库配置缺陷利用、Agent Job 命令执行以及 Windows 服务账户特权滥用等域渗透核心技能。
...
![](/images/maze-faker/8.png)

可以看到下载请求

![](/images/maze-faker/9.png)

验证发现temp目录似乎不可写，根目录也不可写

![](/images/maze-faker/11.png)
