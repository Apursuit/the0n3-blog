...
sidebar: true
description: >-
  MazeSec Watcher 靶机 Writeup：JWT密钥爆破、SQLite报错注入、inotifywait监控滥用、PATH环境变量劫持、Linux提权。通过识别Flask应用的JWT认证机制，利用弱密钥maze伪造管理员Token进入后台；在配置更新接口发现SQLite逻辑注入，通过子查询拼接报错获取数据库凭据，登录watcher用户；借助pspy监控发现root权限运行的inotifywait同步脚本，利用脚本中export PATH="$WATCH_DIR:$PATH"的缺陷实现PATH劫持；通过在可写目录创建恶意sleep脚本覆盖系统命令，等待监控脚本触发后执行任意代码，最终获取root权限。该靶机重点考察了JWT安全、SQLite注入技巧、Linux文件系统监控机制及环境变量劫持等综合渗透能力。
---
...
**关键词**：JWT密钥爆破、SQLite报错注入、inotifywait监控滥用、PATH环境变量劫持、Linux提权

**摘要**：本文记录了MazeSec Watcher靶机的渗透过程。通过识别Flask应用的JWT认证机制，利用弱密钥maze伪造管理员Token进入后台；在配置更新接口发现SQLite逻辑注入，通过子查询拼接报错获取数据库凭据，登录watcher用户；借助pspy监控发现root权限运行的inotifywait同步脚本，利用脚本中export PATH="$WATCH_DIR:$PATH"的缺陷实现PATH劫持；通过在可写目录创建恶意sleep脚本覆盖系统命令，等待监控脚本触发后执行任意代码，最终获取root权限。该靶机重点考察了JWT安全、SQLite注入技巧、Linux文件系统监控机制及环境变量劫持等综合渗透能力。
