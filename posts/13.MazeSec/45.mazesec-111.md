---
title: MazeSec 111
date: 2026-01-09 15:15:34
permalink: /pages/maze-111/
categories:
  - MazeSec
tags:
  - wfuzz
...
description: >-
  MazeSec 111 靶机 Writeup：任意文件读取、Hydra 弱口令爆破、sudo wfuzz 文件写入、sudo id 覆写执行、换行符注入提权。选手通过 80 端口的 file.php 发现任意文件读取漏洞，读取 /etc/passwd 确认系统存在 tao 用户；使用 Hydra 配合 rockyou.txt 字典爆破 SSH，成功获取 tao 用户的凭证（密码 rockyou）；登录后通过 sudo -l 发现 tao 拥有 NOPASSWD 权限执行 /usr/bin/wfuzz 与 /usr/bin/id。利用 sudo wfuzz 的 -f 参数将扫描结果写入 /usr/bin/id 文件，通过三种可控点实现覆写：一是利用命令行引号未闭合产生的换行符注入，在日志中写入 bash 触发 root shell；二是利用字典内容中的反引号命令替换（bash），在 shell 展开阶段优先执行；三是利用 shell 对包含 / 的命令名直接作为路径解析的特性，在当前目录预创建对应路径的可执行文件进行劫持。最终 sudo 执行被覆写的 /usr/bin/id 时，由 /bin/sh 按行解释该文本文件，在执行到 bash 所在行时启动 root 权限的 Shell，完成权限提升。该靶机重点考察了 sudo 二进制程序 wfuzz 的输出文件滥用、Linux shell 解析特性（换行注入、命令替换、路径解析）以及 sudo 配置缺陷导致的提权技巧。
---
...
**关键词**：任意文件读取、Hydra 弱口令爆破、sudo wfuzz 文件写入、sudo id 覆写执行、换行符注入提权

**摘要**：本文记录了 MazeSec 111 靶机的渗透过程。选手通过 80 端口的 file.php 发现任意文件读取漏洞，读取 /etc/passwd 确认系统存在 tao 用户；使用 Hydra 配合 rockyou.txt 字典爆破 SSH，成功获取 tao 用户的凭证（密码 rockyou）；登录后通过 sudo -l 发现 tao 拥有 NOPASSWD 权限执行 /usr/bin/wfuzz 与 /usr/bin/id。利用 sudo wfuzz 的 -f 参数将扫描结果写入 /usr/bin/id 文件，通过三种可控点实现覆写：一是利用命令行引号未闭合产生的换行符注入，在日志中写入 bash 触发 root shell；二是利用字典内容中的反引号命令替换（bash），在 shell 展开阶段优先执行；三是利用 shell 对包含 / 的命令名直接作为路径解析的特性，在当前目录预创建对应路径的可执行文件进行劫持。最终 sudo 执行被覆写的 /usr/bin/id 时，由 /bin/sh 按行解释该文本文件，在执行到 bash 所在行时启动 root 权限的 Shell，完成权限提升。该靶机重点考察了 sudo 二进制程序 wfuzz 的输出文件滥用、Linux shell 解析特性（换行注入、命令替换、路径解析）以及 sudo 配置缺陷导致的提权技巧。
...
![](/images/maze-111/8.png)

![](/images/maze-111/9.png)
...
这里的 `bash` 实际上会捕获子进程的所有标准输出，在 子进程使用`exit`结束进程时替换掉 `bash`，所以实际上执行的是子进程的标准输出 `111`，而子进程执行过程遇到的错误会直接输出到终端
...
![](/images/maze-111/18.png)

把标准输出重定向到标准错误，就能看到回显了

![](/images/maze-111/19.png)
...
ta0 方案：当 shell 解析一行命令时，如果“命令名”中包含 /，shell 不会再走 $PATH 查找，而是直接把它当成路径执行。
...
shell 会把：第一个 token (command)，视为要执行的命令，后面的内容视为参数
...
shell 不会去 $PATH 中查找，而是直接把它当成一个文件路径执行。
...
因此，如果我们能控制当前目录，并提前构造对应路径和可执行文件，就可能让日志中的某一行被 shell 当成命令执行。
...
![](/images/maze-111/15.png)

![](/images/maze-111/16.png)
