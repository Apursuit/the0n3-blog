...
description: >-
  一台 Linux 提权靶机，通过二维码上传功能触发 PHP 命令注入获取初始权限，结合异常二进制文件分析获取用户凭证，最终利用 Bash 算术解析注入绕过 sudo 脚本限制完成权限提升。
---
...
**关键词**：Bash算术解析注入、命令注入、PHP代码审计、sudo提权、Linux提权

**摘要**：一台 Linux 提权靶机，通过二维码上传功能触发 PHP 命令注入获取初始权限，结合异常二进制文件分析获取用户凭证，最终利用 Bash 算术解析注入绕过 sudo 脚本限制完成权限提升。
...
![](/images/mazesec-115final/1.png)

可以在 CyberChef 中的 Generate QR Code 模块生成二维码

![](/images/mazesec-115final/2.png)
...
![](/images/mazesec-115final/3.png)

尝试使用反引号执行命令测试，命令执行成功

![](/images/mazesec-115final/4.png)
...
![](/images/mazesec-115final/5.png)

## 异常二进制文件到密码发现

在靶机执行 dpkg -V （用于校验已安装包文件的完整性），发现 /bin/ps 文件校验失败，疑似被修改

![](/images/mazesec-115final/6.png)
...
![](/images/mazesec-115final/7.png)

SSH 登录成功

![](/images/mazesec-115final/8.png)

## sudo 脚本分析

用户 suraxddq 有 sudo 权限，并且设置了 secure_path 环境变量，因此脚本里即使存在相对路径命令也不能劫持

![](/images/mazesec-115final/9.png)
...
![](/images/mazesec-115final/10.png)
...
![](/images/mazesec-115final/11.png)